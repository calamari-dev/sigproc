...
集合と距離の組\((S,d)\)を\termdef{距離空間}\index{きょりくうかん@距離空間}\index{\((S,d)\)}（metric space）という．
...
\begin{definition}{ノルム}{norm}\index{のるむ@ノルム}\index{\(\vnorm{\holder}\)}
...
\begin{proof}
  ...
  集合\(A_n=\Set{\vect{y}\in C\given\vnorm{\vect{y}-\vect{x}}^2<\delta^2+1/n}\)は各\(n=1,2,\dotsc\)に対して空ではない．
  ...
  \]
  である．よって，\(\seq{\vect{a}_n}\)はコーシー列なので極限点\(\vect{m}\)が存在する．\(C\)は閉集合だから\(\vect{m}\in C\)で，\nameref{proposition:norm_continuous}より\(\vnorm{\vect{x}-\vect{m}}=\lim_{n\to\infty}\vnorm{\vect{x}-\vect{a}_n}=\delta\)である．
  したがって，\(\delta\)の定義から\(\vnorm{\vect{x}-\vect{m}}=\min_{\vect{y}\in C}\vnorm{\vect{x}-\vect{y}}\)である．
