...
\cref{definition:metric_space}のように抽象的な形で距離を定義する利点の1つは，\(\numset{K}^n\)以外の集合に対しても，点列の極限を定義できることである．
...
\begin{example}[一様収束]
  ...
  実際\(d(f_n,\phi)=\max\Set{\abs{f_n(x)}\given x\in I}=1/n\)なので，\(n\)を十分大きくとれば\(d(f_n,\phi)\)の値を限りなく小さくできる（\cref{figure:uniform_converge}）．
\end{example}
...
\begin{definition}{閉包}{closure}\index{へいほう@閉包}\index{\(\clsr A\)}
  \((S,d)\)を距離空間とする．集合\(A\subset S\)に対して，\(A\)上の収束列すべての極限点からなる集合を\(A\)の\termdef{閉包}（closure）といい，\(\clsr A\)と書く\footnotemark ．
\end{definition}
...
\begin{proof}
  ...
  集合\(A_n=\Set{\vect{y}\in C\given\vnorm{\vect{x}-\vect{y}}^2<\delta^2+1/n}\)は各\(n=1,2,\dotsc\)に対して空ではない．
  そこで，\(C\)上の点列\(\seq{\vect{a}_n}\)を各\(n\)に対して\(\vect{a}_n\in A_n\)となるようにとれる．

  \(\seq{\vect{a}_n}\)がコーシー列であることを示す．\nameref{theorem:median_theorem}より，任意の\(m,n\in\numset{N}\)について
  \begin{align*}
    \vnorm{\vect{a}_m-\vect{a}_n}^2 &= \vnorm{(\vect{a}_m-\vect{x})-(\vect{a}_n-\vect{x})}^2 \\
    &= 2(\vnorm{\vect{a}_m-\vect{x}}^2+\vnorm{\vect{a}_n-\vect{x}}^2)-\vnorm{(\vect{a}_m-\vect{x})+(\vect{a}_n-\vect{x})}^2 \\
    &= 2\vnorm{\vect{a}_m-\vect{x}}^2+2\vnorm{\vect{a}_n-\vect{x}}^2-4\vnorm*{\frac{\vect{a}_m+\vect{a}_n}{2}-\vect{x}}^2
  \end{align*}
  が成立する．\(\vect{a}_m\in A_m\)，\(\vect{a}_n\in A_n\)かつ，\(C\)は凸集合だから\((\vect{a}_m+\vect{a}_n)/2\in C\)である．したがって
  \[
  ...
  \]
  である．よって\(\seq{\vect{a}_n}\)はコーシー列なので，極限点\(\vect{m}\)が存在する．\(C\)は閉集合だから\(\vect{m}\in C\)で，\nameref{proposition:norm_continuous}より\(\vnorm{\vect{x}-\vect{m}}=\lim_{n\to\infty}\vnorm{\vect{x}-\vect{a}_n}=\delta\)である．
...
\begin{theorem}{ベッセルの不等式}{bessels_inequality}\index{べっせるのふとうしき@ベッセルの不等式}
  \(H\)をヒルベルト空間とする．\(H\)上の点列\(\seq{\vect{e}_n}\)が正規直交列なら，任意の\(\vect{x}\in H\)に対して次式が成立する．
  \begin{equation}
...
\begin{proof}
  ...
  \(V=\bigcup_{n=1}^\infty V_n\)（\(V_n=\spannedby\Set{\vect{v}_1,\dots,\vect{v}_n}\)）なので，各\(\vect{v}_n\)は次のように表せる．
  \begin{equation}
    \label{equation:closure_approx_sequence}
    \vect{v}_n = \sum_{k=1}^{d_n}\midx{z}{n}{k}\vect{e}_k\quad\text{(\(d_n=\min\Set{k\in\numset{N}\given\vect{v}_n\in V_k}\)，\(\midx{z}{n}{k}\in\numset{C}\))}
  \end{equation}

  \(\vect{p}=\sum_{m=1}^\infty\innerp{\vect{x}}{\vect{e}_m}\vect{e}_m\)とする．\nameref{proposition:inner_products_continuous}から
  \begin{align*}
    \innerp{\vect{x}-\vect{p}}{\vect{v}} &= \innerp[\bigg]{\vect{x}-\sum_{m=1}^\infty\innerp{\vect{x}}{\vect{e}_m}\vect{e}_m}{\lim_{n\to\infty}\sum_{k=1}^{d_n}\midx{z}{n}{k}\vect{e}_k} \\
    &= \lim_{n\to\infty}\sum_{k=1}^{d_n}\conj{\midx{z}{n}{k}}\innerp[\bigg]{\vect{x}-\sum_{m=1}^\infty\innerp{\vect{x}}{\vect{e}_m}\vect{e}_m}{\vect{e}_k} \\
    &= \lim_{n\to\infty}\sum_{k=1}^{d_n}\conj{\midx{z}{n}{k}}\pqty[\bigg]{\innerp{\vect{x}}{\vect{e}_k}-\sum_{m=1}^\infty\innerp{\vect{x}}{\vect{e}_m}\innerp{\vect{e}_m}{\vect{e}_k}}
  \end{align*}
  であり，\(\innerp{\vect{e}_m}{\vect{e}_k}=\kdelta{m}{k}\)より\(\innerp{\vect{x}-\vect{p}}{\vect{v}}=0\)である．
