...
\begin{definition}{閉包}{closure}\index{へいほう@閉包}\index{しーえる@\(\clsr A\)}
...
\begin{definition}{直交射影}{hilbert_projection}\index{ちょっこうしゃえい@直交射影!ひるべるとくうかん@ヒルベルト空間}\index{ぷろじぇくしょん@\(\proj_V(\vect{x})\)}
...
\begin{definition}{正規直交系，完全正規直交系}{ons}\index{せいきちょっこうけい@正規直交系}\index{かんぜんせいきちょっこうけい@完全正規直交系}\index{せいきちょっこうけい@正規直交系!かんぜん@完全\twoemdash}
  \(H\)をヒルベルト空間，\(\seq{\vect{e}_n}\)を\(H\)上の点列とする．
  \begin{enumerate}
    \item \(\innerp{\vect{e}_i}{\vect{e}_j}=\kdelta{i}{j}\)（\(i,j\in\numset{N}\)）であるとき，\(\seq{\vect{e}_n}\)は\termdef{正規直交系}（orthonormal system; ONS）であるという
    \item \(\seq{\vect{e}_n}\)が正規直交系かつ\(\clsr(\spannedby\Set{\vect{e}_1,\vect{e}_2,\dotsc})=H\)であるとき，\(\seq{\vect{e}_n}\)は\termdef{完全正規直交系}（complete orthonormal system; CONS）であるという
  \end{enumerate}
\end{definition}
