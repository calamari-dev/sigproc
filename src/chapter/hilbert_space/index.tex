...
\begin{theorem}{凸射影定理}{convex_projection_theorem}\index{とつしゃえいていり@凸射影定理}\index{しゃえいていり@射影定理!とつ@凸\texttwoemdash}
  ...
  このとき，\(\argmin_{y\in C}\vnorm{y-x}\)はただ一つの元からなる集合である．
\end{theorem}
...
\begin{proof}
  閉部分空間は閉凸集合だから，\nameref{theorem:convex_projection_theorem}より\(n\in\argmin_{y\in V}\vnorm{y-x}\)を満たす\(n\)が一意に定まる．あとは\cref{xr-proposition:finite_projection}と同様に示せる．
\end{proof}
...
\begin{proof}
  ...
  \[
    \innerp{\epsilon}{\phi_j} = \innerp*{\sum_{i=1}^\infty\innerp{x}{\phi_i}\phi_i-x}{\phi_j}
    = \sum_{i=1}^\infty\innerp{x}{\phi_i}\innerp{\phi_i}{\phi_j}-\innerp{x}{\phi_j}
    = 0
  \]
