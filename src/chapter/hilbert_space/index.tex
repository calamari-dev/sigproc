...
\begin{definition}{閉包・閉集合・稠密}{closure}\index{へいほう@閉包}\index{へいしゅうごう@閉集合}\index{ちゅうみつ@稠密}\index{しーえる@\(\clsr A\)}
  ...
  \begin{enumerate}
    ...
    \item 集合\(B\subset A\)が\(\clsr B=A\)を満たすとき，\(B\)は\(A\)において\ltjruby{\termdef{稠密}}{ちゆうみつ}（dense）であるという
  \end{enumerate}
\end{definition}
...
\begin{example}[\(\lebesgue{p}\)空間]
  ...
  バナッハ空間\(\lebesgue{p}(\numset{N})\)を\termdef{\(\lebesgue{p}\)空間}\index{えるぴーくうかん@\(\lebesgue{p}\)空間}（\(\lebesgue{p}\) space）という．
\end{example}
