...
\begin{proof}
  ...
  \(V=\bigcup_{n=1}^\infty\spannedby\basis{B}_n\)（\(\basis{B}_n=\Set{\vect{e}_1,\dots,\vect{e}_n}\)）なので，各\(n\in\numset{N}\)に対して\(\vect{v}_n\in\spannedby\basis{B}_{d(n)}\)を満たす\(d(n)\in\numset{N}\)をとれて，
  \(\vect{v}_n\)は\(\basis{B}_{d(n)}\)の元の線型結合で\(\vect{v}_n=\midx{z}{n}{1}\vect{e}_1+\dots+\midx{z}{n}{d(n)}\vect{e}_{d(n)}\)とおける．
  ...
  \begin{gather*}
    ...
    = 0, \\
    \innerp{\vect{x}-\vect{p}}{\vect{v}} = \innerp*{\vect{x}-\vect{p}}{\lim_{n\to\infty}\sum_{k=1}^{d(n)}\midx{z}{n}{k}\vect{e}_k}
    = \lim_{n\to\infty}\sum_{k=1}^{d(n)}\conj{\midx{z}{n}{k}}\innerp{\vect{x}-\vect{p}}{\vect{e}_k}
    = 0
  \end{gather*}
