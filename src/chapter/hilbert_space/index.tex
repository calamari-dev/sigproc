...
\begin{proof}
  ...
  そこで，\(C\)上の点列\(\seq{a_n}\)を各\(n\)に対して\(a_n\in A_n\)となるように選べる．
  \(0\leq\vnorm{x-a_n}-\delta\leq 1/n\to 0\)（\(n\to\infty\)）なので，\(\seq{a_n}\)がある点\(a_\infty\)に収束すれば，\nameref{proposition:norm_continuous}より
  \(\vnorm{x-a_\infty}=\lim_{n\to\infty}\vnorm{x-a_n}=\delta\)である．さらに，\(C\)は閉集合だから\(a_\infty\in C\)，よって\(a_\infty\in\argmin_{y\in C}\vnorm{x-y}\)である．
  ...
  このとき\(\vnorm{x-y_1}=\vnorm{x-y_2}=\delta\)，\((y_1+y_2)/2\in C\)なので\(\vnorm{y_1-y_2}^2=2\delta^2+2\delta^2-4\vnorm{x-(y_1+y_2)/2}^2\leq 0\)，したがって\(y_1=y_2\)である．
\end{proof}
