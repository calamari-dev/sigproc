...
\begin{definition}{閉包・閉集合・稠密}{closure}\index{へいほう@閉包}\index{へいしゅうごう@閉集合}\index{ちゅうみつ@稠密}\index{しーえる@\(\clsr S\)}
...
\footnotetext{本書では閉包を\(\clsr A\)，補集合を\(\scomp{A}\)\index{しー@\(\scomp{S}\)}で表す．}
