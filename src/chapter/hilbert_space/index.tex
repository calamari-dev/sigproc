...
\begin{proof}
  ...
  \(0\leq\vnorm{x-a_n}-\delta\leq 1/n\to 0\)（\(n\to\infty\)）なので，\(\seq{a_n}\)の極限点\(a_\infty\)が存在すれば，\nameref{proposition:norm_continuous}より\(\vnorm{x-a_\infty}=\lim_{n\to\infty}\vnorm{x-a_n}=\delta\)である．
  \(C\)は閉集合だから\(a_\infty\in C\)，よって\(a_\infty\in\argmin_{y\in C}\vnorm{x-y}\)である．
...
\begin{proof}
  ...
  \(v\in\clsr V\)なので，\(\vnorm{v-\tilde{v}_n}\to 0\)（\(n\to\infty\)）を満たす\(V\)上の点列\(\seq{\tilde{v}_n}\)がある．
  ...
  したがって，\nameref{theorem:cauchy_schwarz}より\(\abs{\innerp{\epsilon}{v}}\leq\vnorm{\epsilon}\vnorm{v-\tilde{v}_n}\to 0\)（\(n\to\infty\)），\(\innerp{\epsilon}{v}=0\)である．
\end{proof}
