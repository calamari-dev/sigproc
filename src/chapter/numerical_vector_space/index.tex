...
\begin{definition}{\(\argmin\)，\(\argmax\)}{argmin_argmax}\index{あーぎゅみん@\(\argmin f(x)\)}\index{あーぎゅまっくす@\(\argmax f(x)\)}
...
\begin{definition}{直交射影}{finite_projection}\index{ちょっこうしゃえい@直交射影}\index{ぷろじぇくしょん@\(\proj_V(\vect{x})\)}
...
\begin{definition}{離散フーリエ変換}{discrete_fourier_transform}\index{りさんふーりえへんかん@離散フーリエ変換}\index{でぃーえふてぃー@DFT|see{離散フーリエ変換}}\indexsymbol{\(\dft{n}\vect{x}\)}
  各\(\vect{x}=\trps{\rowvect{x_1 & \cdots & x_n}}\in\numset{C}^n\)に対して
  \[
    \dft{n}{\vect{x}} = \trps{\matrice{y_1 & \cdots & y_n}},
    \quad y_j = \frac{1}{\sqrt{n}}\sum_{k=1}^nx_k\napr^{-2\krez\iuni jk/n}\quad(j=1,\dots,n)
  \]
