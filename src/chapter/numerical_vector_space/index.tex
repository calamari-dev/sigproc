...
実験で得られるデータは誤差を含むものだから，普通は標本を\(p+1\)個より多く測定し，多項式関数\(f(\vect{c};x)=c_0+c_1x+\dots+c_px^p\)のあてはまりが平均的によくなる組\(\vect{c}=\trps{\rowvect{c_0 & \cdots & c_p}}\)を求める（図は標本と\(f(\vect{c};x)\)の一例である）．
