...
\begin{definition}{内積}{inner_product}\index{ないせき@内積}\index{ないせきくうかん@内積空間}\index{\(\innerp{\holder}{\holder}\)}
  ...
  また，\(V\)に内積が定義されているとき，\(V\)は\termdef{内積空間}（inner product space）であるという．
\end{definition}
...
\(\innerp{\vect{v}}{\vect{w}}=0\)であるとき，ベクトル\(\vect{v}\)と\(\vect{w}\)は\termdef{直交}\index{ちょっこう@直交}するという．

\begin{definition}{正規直交基底}{onb}\index{せいきちょっこうきてい@正規直交基底}
  \(V\)を内積空間とする．\(V\)の基底\(\basis{B}\)が\termdef{正規直交基底}（orthonormal basis）であるとは，任意の\(\vect{e}_1,\vect{e}_2\in\basis{B}\)が条件
  \[
...
\begin{example}
  行列\(\mat{R}=\begin{bsmallmatrix}1/2 & -1/\sqrt{2} & 1/2 \\ 1/\sqrt{2} & 0 & -1/\sqrt{2} \\ 1/2 & 1/\sqrt{2} & 1/2\end{bsmallmatrix}\)を各\(\vect{x}\in\numset{R}^3\)に左から掛けると，\(\vect{x}\)があるベクトル\(\vect{v}\)を軸に角度\(\theta\)だけ回転する．
  \(\vect{v}\)と\(\theta\)を求めよう．

  回転軸上にあるベクトルは\(\mat{R}\)を掛けても回転しないから，\(\vect{v}\)は\(\mat{R}\vect{v}=\vect{v}\)を満たす．
  よって，\(\vect{v}\)を求めるには，連立1次方程式\((\mat{R}-\imat)\vect{v}=\zvec\)を解けばよい．
  この解は不定であり，一般解は\(\vect{v}=t\trps{\inlinevec{1 & 0 & 1}}\)（\(t\in\numset{R}\)）となる．

  以下では特に，\(\vect{v}\)の長さが1となる\(\vect{v}_1=\trps{\inlinevec{1 & 0 & 1}}/\sqrt{2}\)の場合について考える．
  \(\vect{v}_1\)と直交する2ベクトル\(\vect{v}_2=\trps{\inlinevec{0 & 1 & 0}}\)，\(\vect{v}_3=\trps{\inlinevec{-1 & 0 & 1}}/\sqrt{2}\)をとると，
  集合\(\basis{B}=\Set{\vect{v}_1,\vect{v}_2,\vect{v}_3}\)は\(\numset{R}^3\)の基底になる．
\end{example}
