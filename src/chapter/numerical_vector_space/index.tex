...
あてはまりのよさを表現する指標はいろいろ考えられるが，ここでは関数
\[
  \epsilon(\vect{c}) = \frac{1}{n}\sum_{i=1}^n\abs*{y_i-f(\vect{c};x_i)}^2
\]
を採用する．

\begin{note}
  差の2乗であてはまりのよさを測るのが妥当かどうかは，データの性質や分析する目的にもよる．
  しかし，ほかの指標に関して\(\argmin\)を計算するのは\texttwoemdash それ以前に，空集合でないことを保証するだけでも\texttwoemdash 難しいことが多い．
  そのため，まずは差の2乗が使えないか考えてみるべきだろう．
\end{note}
