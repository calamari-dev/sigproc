...
\begin{note}
  普通，特異値分解はもとの行列に対して一意ではない．しかし，擬似逆行列はもとの行列に対して一意に定まる．証明は\cref{subsection:pseudoinverse}を参照せよ．
\end{note}
...
\begin{proposition}{擬似逆行列の特徴づけ}{pseudoinverse_characterization}
  ...
  \begin{enumerate}
    \item \(\mat{A}\mat{X}\mat{A}=\mat{A}\)である．
    \item \(\mat{X}\mat{A}\mat{X}=\mat{X}\)である．
    \item \(\mat{A}\mat{X}\)はエルミート行列である．
    \item \(\mat{X}\mat{A}\)はエルミート行列である．
  \end{enumerate}
\end{proposition}
...
\begin{proof}
  ...
  である．よって\(\proj_{\colsp\mat{A}}\vect{x}=\mat{A}\pinv{\mat{A}}\vect{x}\)である．後半は\(\proj_{\pcomp{(\colsp\mat{A})}}\vect{x}=\vect{x}-\proj_{\colsp\mat{A}}\vect{x}\)から分かる．
\end{proof}
...
\begin{proposition}{}{}
  ...
  すなわち\(\Set{\vect{w}}=\argmin_{\vect{x}\in S}\vnorm{\vect{x}}\)である．
\end{proposition}
