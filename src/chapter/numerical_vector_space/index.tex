...
\begin{note}
  \cref{equation:discrete_impulse_response}より，\(T\)の多項式\(X(T)=\sum x[m]T^m\)，\(Y(T)=\sum y[m]T^m\)に関して\(x\circconv y=X(\lagop)y=X(\lagop)Y(\lagop)\delta\)が成立する．
  実は，\(0\leq n<N\)のとき\((x\circconv y)[n]\)は，\(X(T)Y(T)\)を\(T^N-1\)で割ったあまりの\(T^n\)の係数である．
  実際，商を\(Q(T)\)，あまりを\(R(T)=\sum r_mT^m\)とすれば，\(X(T)Y(T)=Q(T)(T^N-1)+R(T)\)と\(\lagop^N=\lagop^0\)から\(X(\lagop)Y(\lagop)\delta=R(\lagop)\delta=\sum r_m\lagop^m\delta\)が分かる．
\end{note}
...
なお，エイリアシングが起きている\(k>16\)でグラフはどうなっているかというと，次の命題から直線\(k=16\)に関して鏡写しになっている．
...
\cref{figure:pudding_dft}は，画像を離散信号\(s=\seq{s[x,y]}\)とみなし\(\abs{\hat{s}[u,v]}\)を計算した結果である．
...
この筋は\cref{figure:pudding_dft}のプリンの画像が，その方向に関して周期性を持つことを示している．
...
\begin{figure}[htbp]
  ...
  \end{minipage}
  \caption{\(\cos(3x+4y)\)（\(x,y\in\Set{2\krez k/64\given k=0,\dots,63}\)）のグラフと2次元DFT．左図を座標空間で描くとトタン屋根のような形になる．また，右図の2点は\(\pm\smallmatrice{3 \\ 4}\)にある．}
  \label{figure:2dcos_dft}
\end{figure}

\cref{figure:2dcos_dft}は\(\vect{n}=\smallmatrice{64 \\ 64}\)，\(\vect{k}=\smallmatrice{3 \\ 4}\)のときの\(\cos(\trps{\vect{k}}\mat{N}^{-1}\vect{r})=\sqrt{\det\mat{N}}\rpart w_{\vect{k}}[\vect{r}]\)のグラフと，その2次元離散フーリエ変換である．
...
\cref{figure:pudding_shift,figure:2dcos_dft}を見比べると，最も長い光の筋はだいたい\(\smallmatrice{3 \\ 4}\)方向を向いている．
