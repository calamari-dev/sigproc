...
\begin{note}
  実は\cref{proposition:frame_characterization}の逆も成り立つ．すなわち，有限集合\(S\)が\(H\)の生成系なら，\(S\)の各元に通し番号を振ると\(H\)のフレームになる（Casazza \cite{casazza2013}に証明がある）．
\end{note}
...
フレームの一般論を交えて，STFTが持つ性質を見ていこう．

窓\(w\in\cycles{L}\)は零でないとする．\(I=\Set{(k,n)\given k,n\in\Set{0,\dots,L-1}}\)とおく．

\begin{proposition}{}{}
  列\(\seq{\modulate{k}\translate{n}w}_{(k,n)\in I}\)は\(\cycles{L}\)のタイトフレームである．
\end{proposition}
