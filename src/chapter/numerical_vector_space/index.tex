...
基底でない生成系を\termdef{\ltjruby{冗長}{じようちよう}}\index{じょうちょう@冗長}（redundant）な生成系という．フレームを考える動機の1つは，正規直交基底による分解・合成を冗長な生成系に拡張することである．
...
\begin{lemma}{}{grammian_equals_zero}
  \(G\)は\(H\)上の線形変換で，任意の\(\vect{x},\vect{y}\in H\)に対し\(\innerp{G(\vect{x})}{\vect{y}}=\innerp{\vect{x}}{G(\vect{y})}\)かつ\(\innerp{G(\vect{x})}{\vect{x}}=0\)であるとする．
  このとき\(\img G=\Set{\zvec}\)である．
\end{lemma}

\begin{proof}
  任意の\(\vect{x},\vect{y}\in H\)に対して\(\innerp{G(\vect{x}+\vect{y})}{\vect{x}+\vect{y}}=\innerp{G(\vect{x})}{\vect{y}}+\innerp{G(\vect{y})}{\vect{x}}=\innerp{G(\vect{x})}{\vect{y}}+\innerp{\vect{y}}{G(\vect{x})}=0\)だから，
...
\(\cycles{L}\)上の列
\[
  \gaborsys{w}{K}{N} = \seq{\modulate{iK}\translate{\:jN}w}_{(i,j)\in\Lambda}\quad(\Lambda=\Set{(i,j)\given iK,jN\in J})
\]
を\termdef{ガボールシステム}\index{がぼーるしすてむ@ガボールシステム}\index{GwKN@\(\gaborsys{w}{K}{N}\)}（Gabor system）という．
\(\gaborsys{w}{K}{N}\)が\(\cycles{L}\)のフレームなら，その分解作用素\(T\)は\(T(x)[i,j]=(\dgt{w})[i,j]\)を満たす．しかし，パラメータ\(w\)・\(K\)・\(N\)・\(L\)の組み合わせによっては\(\gaborsys{w}{K}{N}\)がフレームにならないこともある．

\begin{example}
  \(L=N\geq 2\)のとき，\(w[n]=0\)となる\(n\)の値が1つでもあると\(\gaborsys{w}{K}{N}\)はフレームにならない（\(N=3\)などで確かめよ）．
\end{example}
