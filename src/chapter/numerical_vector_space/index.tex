...
\begin{proof}
  ...
  \(\vect{x}\)を\(\increment\vect{x}\)だけ動かしたとき，\(\epsilon(\vect{x})\)の増分\(\increment\epsilon(\vect{x})=\epsilon(\vect{x}+\increment\vect{x})-\epsilon(\vect{x})\)は
  \begin{align*}
    ...
    &= -2\rpart\innerp{\htrps{\mat{A}}\vect{\epsilon}(\vect{x})}{\increment\vect{x}}+\vnorm{\mat{A}\increment\vect{x}}^2
    = -2\rpart\innerp{\vect{g}}{\increment\vect{x}}+\vnorm{\mat{A}\increment\vect{x}}^2
  \end{align*}
  である．よって，\(\vect{g}=\zvec\)ならば\(\increment\vect{x}\)の値によらず\(\increment\epsilon(\vect{a})=\vnorm{\mat{A}\increment\vect{x}}^2\geq 0\)だから，\(\epsilon\)は点\(\vect{a}\)で最小値をとる．
...
\cref{figure:2dcos_dft}は\(\vect{n}=\smallmatrice{64 \\ 64}\)，\(\vect{k}=\smallmatrice{3 \\ 4}\)のときの\(\cos(\trps{\vect{k}}\mat{N}^{-1}\vect{r})=\sqrt{\det\mat{N}}\rpart w_{\vect{k}}[\vect{r}]\)のグラフと，その2次元DFTである．
