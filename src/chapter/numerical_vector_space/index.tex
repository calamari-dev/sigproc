...
\begin{proof}
  ...
  \begin{align*}
    \vnorm{\vect{y}-\vect{x}}^2 &= \vnorm*{\vect{x}-\sum z_i\vect{e}_i}^2
  ...
  \(\argmin_{\vect{y}\in V}\vnorm{\vect{y}-\vect{x}}=\Set{\sum\innerp{\vect{x}}{\vect{e}_i}\vect{e}_i}\)である．
\end{proof}
...
\begin{proof}
  ...
  \(\vect{m}\in\argmin_{\vect{y}\in V}\vnorm{\vect{y}-\vect{x}}\)のとき，すべての\(\vect{v}\in V\)に対して\(\innerp{\vect{m}-\vect{x}}{\vect{v}}=0\)が成り立つことを示す．それには\(\vnorm{\vect{v}}=1\)のときについて示せば十分である．
  \(\vect{m}\in\argmin_{\vect{y}\in V}\vnorm{\vect{y}-\vect{x}}\)なので，関数\(\delta(z)=\vnorm{(\vect{m}+z\vect{v})-\vect{x}}^2-\vnorm{\vect{m}-\vect{x}}^2\)（\(z\in\numset{C}\)）は負の値をとらない．
  一方，\(x=\rpart z\)，\(y=\ipart z\)とおくと
  \begin{align*}
    \delta(z) &= \vnorm{(\vect{m}-\vect{x})+z\vect{v}}^2-\vnorm{\vect{m}-\vect{x}}^2
    = 2\rpart(\conj{z}\innerp{\vect{m}-\vect{x}}{\vect{v}})+\abs{z}^2\vnorm{\vect{v}}^2 \\
    &= 2(x\rpart\innerp{\vect{m}-\vect{x}}{\vect{v}}+y\ipart\innerp{\vect{m}-\vect{x}}{\vect{v}})+x^2+y^2 \\
    &= (x+\rpart\innerp{\vect{m}-\vect{x}}{\vect{v}})^2+(y+\ipart\innerp{\vect{m}-\vect{x}}{\vect{v}})^2-\abs{\innerp{\vect{m}-\vect{x}}{\vect{v}}}^2 \\
    &= \abs{z+\innerp{\vect{m}-\vect{x}}{\vect{v}}}^2-\abs{\innerp{\vect{m}-\vect{x}}{\vect{v}}}^2
  \end{align*}
  より\(-\abs{\innerp{\vect{m}-\vect{x}}{\vect{v}}}^2=\delta(-\innerp{\vect{m}-\vect{x}}{\vect{v}})\geq 0\)，よって\(\innerp{\vect{m}-\vect{x}}{\vect{v}}=0\)である．
...
\begin{definition}{直交補空間}{numerical_perpendicular_complement}\index{ちょっこうほくうかん@直交補空間}\indexsymbol{\(\pcomp{V}\)}\indexsymbol{\(\pcomp[V]{W}\)}
  ...
  \]
  も\(V\)の部分空間になる．\(\pcomp{W}\)を（\(V\)における）\(W\)の\termdef{直交補空間}（orthogonal complement）という．
\end{definition}
...
\termdef{最小2乗法}\index{さいしょうにじょうほう@最小2乗法}（least squares method）は，厳密解の代わりに\termdef{残差}\index{ざんさ@残差}（residual）\(\vect{\epsilon}(\vect{x})=\mat{A}\vect{x}-\vect{b}\)のノルム\(\vnorm{\vect{\epsilon(\vect{x})}}\)を最小にする近似解\(\vect{x}\)を見つける手法である．
...
\(\numset{K}^p\)の元\(\vect{x}\)が\(\mat{A}\vect{x}=\proj_{\colsp\mat{A}}\vect{b}\)を満たすとき，\(\vect{a}_j\in\colsp\mat{A}\)なので\(\innerp{\mat{A}\vect{x}-\vect{b}}{\vect{a}_j}=\htrps{\vect{a}_j}(\mat{A}\vect{x}-\vect{b})=0\)である．
よって\(\htrps{\mat{A}}(\mat{A}\vect{x}-\vect{b})=\zvec\)である．
...
人間が知覚する声の高さとかなりよく対応する\index{ピッチ}\footnote{人間が知覚する声の高さを\termdef{ピッチ}（pitch）という．基本周波数とピッチはしばしば同一視されるが，厳密には定義からして異なる量である．詳しくは柏野\cite{kashino}など．}．
...
離散フーリエ変換は曲線あてはめともみなせる．区間\(\torus=\ccival{0}{2\krez}\)上で定義された関数\(x\)を，関数\(\phi_k(t)=\exp(\iuni kt)\)の線型結合\(x(\vect{c};t)=\sum_{k=0}^{N-1}c_k\phi_k(t)\)で近似しよう．
\(t_n=2\krez n/N\)（\(0\leq n<N\)）における\(x\)の値を用いた曲線あてはめ\texttwoemdash すなわち，残差平方和
\[
  \epsilon(\vect{c}) = \sum_{n=0}^{M-1}\abs{x(\vect{c};t_n)-x(t_n)}^2
  = \sum_{n=0}^{N-1}\abs*{x\pqty*{\vect{c};\frac{2\krez n}{N}}-x\pqty*{\frac{2\krez n}{N}}}^2
\]
の最小化\texttwoemdash を行うには，正規方程式\(\htrps{\mat{\Phi}}\mat{\Phi}\vect{c}=\htrps{\mat{\Phi}}\vect{y}\)を解けばよい．ただし
\[
  \mat{\Phi} = \matfence{\phi_{i-1}(t_{j-1})}
...
\]
である．ところが，右辺の行列は\cref{proposition:dft_onb}の正規直交基底を用いて\(\sqrt{N}\mat{W}\)（\(\mat{W}=\rowvect{\vect{w}_0 & \cdots & \vect{w}_{N-1}}\)）と書けるから，
正規方程式の係数行列は\(\htrps{\mat{\Phi}}\mat{\Phi}=N\htrps{\mat{W}}\mat{W}=N\imat\)である．
つまり，正規方程式は立てた時点で解かれているも同然であり，解は\(\vect{c}=(1/N)\htrps{\mat{\Phi}}\vect{y}=(1/\sqrt{N})\htrps{\mat{W}}\vect{y}=(1/\sqrt{N})\dft{N}\vect{y}\)である．

まとめると，\(\torus\)を\(N\)等分した左端における\(x\)の値を標本とし，線型結合
\[
  x(\vect{c};t) = \sum_{k=0}^{N-1}c_k\phi_k(t)
  = \sum_{k=0}^{N-1}c_k\napr^{\iuni kt}
\]
で\(x\)を最もよく近似しようとすると，各係数\(c_k\)は\(\dft{N}\vect{y}\)の定数倍になる．
また，\(\phi_k\)をあらかじめ\(\sqrt{N}\)で割っておけば，\(c_k\)は完全に\(\dft{N}\vect{y}\)と等しくなる．
