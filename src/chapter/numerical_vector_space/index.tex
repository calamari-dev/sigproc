...
\(\mat{A}\)の固有値\(\lambda\)を1つとる．このとき\cref{proposition:eigenspaces_disjointness}より，
固有空間\(E_\lambda(\mat{A})\)上の線型写像\(f_\lambda\colon E_\lambda(\mat{A})\to E_\lambda(\mat{A})\)を\(f_\lambda(\vect{x})=f(\vect{x})=\lambda\vect{x}\)で定義できる．
