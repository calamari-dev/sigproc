...
\cref{section:stft,section:discrete_mra}では，時刻と周波数にまたがって現れる信号の特徴を調べる，\termdef{時間周波数解析}\index{じかんしゅうはすうかいせき@時間周波数解析}（time–frequency analysis）の理論を説明する．この理論は旋律・地震動・景気などを解析する分野で活用されている．
...
誤解する余地がなければ，\(\stft[L]{w}\)を単に\(\stft{w}\)と書く．また，\(w\)を\(\stft{w}\)の\termdef{窓関数}\index{まどかんすう@窓関数}（window function）という．
