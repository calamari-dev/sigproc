...
\cref{corollary:stft_reconstruction}から\(\stft{w}\)は単射である．しかし\(\dim\cycles{L}=L\)，\(\dim\doublecycles{L}=L^2\)なので\texttwoemdash ごく例外的な\(L=1\)の場合を除き\texttwoemdash 全射ではなく，フレーム\(\seq{\modulate{k}\translate{n}w}\)は冗長である．
...
\]
を\termdef{ガボールシステム}\index{がぼーるしすてむ@ガボールシステム}\index{GwKN@\(\gaborsys{w}{K}{N}\)}（Gabor system）という．\(\gaborsys{w}{K}{N}\)が\(\cycles{L}\)のフレームなら，その分解作用素\(T\)は
\[
...
\begin{theorem}{スペクトル定理II}{spectral_theorem_two}\index{すぺくとるていり@スペクトル定理}
  ...
  このとき，\(T\)の固有ベクトルからなる\(H\)の正規直交基底\(\Set{u_1,\dots,u_n}\)（\(n=\dim H\)）が存在する．また，各\(u_k\)が属する\(T\)の固有値を\(\lambda_k\)とおくと
  \[
...
\begin{proof}
  \(\lambda_k>0\)以外は\cref{theorem:spectral_theorem_two}からただちにしたがう．\(T(u_k)=\lambda_ku_k\)より
  \[
    \sum_{i\in I}\abs{\innerp{u_k}{\phi_i}}^2 = \innerp{S(u_k)}{u_k}
    = \innerp{\lambda_ku_k}{u_k}
    = \lambda_k
  \]
  であり，\(\Phi\)はフレームなので，この値は正である．
\end{proof}

\begin{proposition}{}{dual_reconstruction}
  \(\Phi=\seq{\phi_i}_{i\in I}\)を\(H\)のフレーム，\(S_\Phi\)を\(\Phi\)のフレーム作用素とする．このとき，列\(\Psi=\seq{S_\Phi^{-1}(\phi_i)}_{i\in I}\)は\(H\)のフレームである．
...
\begin{proof}[スペクトル定理IIの証明]
  ...
  \[
    ...
    = \trps{\vect{u}_k}(\trps{\mat{S}}\vect{\conj{u}}_l)
    = \trps{(\htrps{\vect{u}_k}(\mat{U}\mat{\Lambda}\htrps{\mat{U}})\vect{u}_l)}
  \]
