...
\begin{proof}
  \(\Omega\)の2元\(\vect{i}=\trps{\rowvect{i_1 & \cdots & i_d}}\)，\(\vect{j}=\trps{\rowvect{j_1 & \cdots & j_d}}\)を任意にとると
  \begin{gather*}
    w_{\vect{i}}[\vect{r}]\conj*{w_{\!\vect{j}}[\vect{r}]} = \frac{\napr^{2\krez\iuni\trps{(\vect{i}-\vect{j})}\mat{N}^{-1}\vect{r}}}{\det\mat{N}}
    = \prod_{k=1}^d\frac{\napr^{2\krez\iuni(i_k-j_k)r_k/N_k}}{N_k}\quad(\vect{r}=\trps{\matrice{r_1 & \cdots & r_d}}), \\
    \innerp{w_{\vect{i}}}{w_{\!\vect{j}}} = \sum_{r_1}\dotsm\sum_{r_d}\prod_{k=1}^d\frac{\napr^{2\krez\iuni(i_k-j_k)r_k/N_k}}{N_k}
    = \prod_{k=1}^d\sum_{r_k}\frac{\napr^{2\krez\iuni(i_k-j_k)r_k/N_k}}{N_k}
  \end{gather*}
  であるから，\cref{proposition:dft_onb}より\(\innerp{w_{\vect{i}}}{w_{\!\vect{j}}}=\prod_{k=1}^d\kdelta{i_k}{j_k}=\kdelta{\vect{i}}{\vect{j}}\)となる\indexsymbol{\(\prod\)}\footnote{\(\prod\)は総乗\(\prod_{i=1}^na_i=a_1\times\dots\times a_n\)を意味する．\(\sum\)の乗法バージョン．}．
\end{proof}
