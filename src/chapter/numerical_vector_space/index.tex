...
\[
  \innerp{x}{y} = \sum_{\vect{r}\in\Omega}x[\vect{r}]\conj*{y[\vect{r}]}
\]
で定義する．
...
\cref{figure:pudding_shift}から光の筋を消して逆変換すると，\cref{figure:flatten}が得られる．
元の画像と\cref{figure:flatten}を見比べれば，光の筋はさくらんぼの枝によるものだと分かる．

\begin{figure}
  \begin{minipage}{\linewidth/2}
    \begin{minipage}[t][40mm][c]{\linewidth}
      \centering
      \includegraphics{figures/flatten_shift.pdf}
    \end{minipage}
    \caption{光の筋を削除した様子．}
  \end{minipage}%
  \begin{minipage}{\linewidth/2}
    \begin{minipage}[t][40mm][c]{\linewidth}
      \centering
      \includegraphics{figures/flatten.pdf}
    \end{minipage}
    \caption{左図の逆変換．}
