...
\begin{corollary}{特異値分解}{singular_value_decomposition}\index{とくいちぶんかい@特異値分解}
  任意の\(n\times p\)複素行列\(\mat{A}\)に対して，以下の命題が成立する．
  \begin{enumerate}
    \item \(\htrps{\mat{A}}\mat{A}\)の固有値はすべて非負であり，正の固有値は重複度を込めて\(r=\rank(\htrps{\mat{A}}\mat{A})\)個ある．
    \item \(\htrps{\mat{A}}\mat{A}\)の正の固有値を\(\lambda_1\geq\dots\geq\lambda_r\)とおく．このとき，\(\mat{A}=\mat{U}\mat{\Sigma}\htrps{\mat{V}}\)を満たす\(n\)次ユニタリ行列\(\mat{U}\)，\(p\)次ユニタリ行列\(\mat{V}\)が存在する．
      ただし，\(\mat{\Sigma}=\smallmatrice{\diag(\sqrt{\lambda_1},\dots,\sqrt{\lambda_r}) & \\ & \zmat}\)は\(n\times p\)行列である．
...
\begin{equation}
  ...
  \sum_{k=0}^{N-1}\abs{(\dft{N}\vect{x})[k]}^2 = \sum_{n=0}^{N-1}\abs{x_n}^2\quad(\vect{x}=\trps{\matrice{x_1 & \cdots & x_n}})
\end{equation}
