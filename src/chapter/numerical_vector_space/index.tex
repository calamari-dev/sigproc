...
\cref{proposition:ddimdft_onb}と\(\hat{x}[\vect{k}]=\innerp{x}{w_{\vect{k}}}\)より，\(\dft[d]{N}\)の逆変換は次のようになる．
\[
...
いくつかの具体例を通して，2次元離散フーリエ変換を体感しよう．
...
\begin{figure}[htbp]
  ...
  \end{minipage}
  \caption{\(\cos(3x+4y)\)（\(x,y\in\Set{2\krez k/50\given k=0,\dots,49}\)）のグラフと2次元DFT．}
  \label{figure:2dcos_dft}
\end{figure}

\cref{figure:2dcos_dft}は，\(\vect{n}=\smallmatrice{50 \\ 50}\)，\(\vect{k}=\smallmatrice{3 \\ 4}\)のときの\(\cos(\trps{\vect{k}}\mat{N}^{-1}\vect{r})=\sqrt{\det\mat{N}}\rpart w_{\vect{k}}[\vect{r}]\)のグラフと，その2次元離散フーリエ変換である．
...
\begin{wrapfigure}[10]{o}{0pt}
  \centering
  \includegraphics{figures/2dcos_shift.pdf}
  \caption{補正後の様子．}
  \label{figure:2dcos_shift}
\end{wrapfigure}

\(\hat{w}_{\vect{k}}[u,v]\)は\(x\)と\(y\)に関して\(w_{\vect{k}}[x,y]\)を離散フーリエ変換したものだから，\(u\geq N/2\)か\(v\geq N/2\)のときはエイリアシングが起きている．
...
この補正により，\cref{figure:2dcos_dft}は\cref{figure:2dcos_shift}に変化する．
...
\cref{figure:pudding_dft}は，\cref{figure:pudding}の離散信号\(s=\seq{s[x,y]}\)に関する\(\abs{\hat{s}[u,v]}\)のグラフである．
ただし\(\vect{n}\)は，画像が\(1063\times 1063\)の画素からなるので\(\vect{n}=\smallmatrice{N \\ N}\)（\(N=1063\)）である．
