...
\begin{note}
  ...
  \[
    \diffp{\epsilon(\vect{x})}{x_i} = \lim_{h\to 0}\frac{\increment\epsilon(\vect{x})}{h}
    = \lim_{h\to 0}\pqty*{2\innerp{\vect{g}}{\vect{e}_i}+\abs{h}\vnorm{\mat{A}\vect{e}_i}^2}
    = 2\innerp{\vect{g}}{\vect{e}_i}
  \]
  である．よって，正規方程式は\(\diffp*{\epsilon(\vect{x})}/{x_i}=0\)（\(1\leq i\leq p\)）とも書ける．
\end{note}
...
スペクトル分解を示す前に，1つ準備をしよう．\(T\colon\numset{K}^m\to\numset{K}^n\)は線型写像で，集合\(V\)は\(\numset{K}^m\)の\(\Set{\zvec}\)でない部分空間とする．
このとき，線型写像\(T_1\colon V\to\numset{K}^n\)，\(T_2\colon\pcomp{V}\to\numset{K}^n\)を\(T_i(\vect{x})=T(\vect{x})\)で定義すると\(T=T_1\proj_V+T_2\proj_{\pcomp{V}}\)が成立する．
