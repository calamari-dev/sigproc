...
さらなる諸性質を導く前に，離散フーリエ変換の工学的重要性を見ておこう．\cref{figure:time_domain}は「あ」という音声の波形である\footnote{出典は波音リツ単独音 Ver1.5.1 \cite{canon}．}．
...
そして，\(x_n\)と\(\hat{x}_k\)の間には\cref{equation:idft}の関係がある．
Lorem ipsum dolor sit amet, consectetur adipiscing elit. Proin rutrum sodales erat, sed tempor tellus mollis a. Donec dapibus turpis non efficitur convallis. Fusce varius, eros nec sodales mattis, ipsum neque varius metus, vel bibendum quam sapien id sapien. Aliquam purus mi, aliquet eu luctus nec, placerat vitae ligula. Nulla vestibulum purus in ornare fermentum. Vestibulum tristique sagittis mi quis commodo. Morbi ex purus, consequat porttitor sagittis eget, lacinia vel sem. Curabitur suscipit lectus eget scelerisque facilisis. Sed ac feugiat lorem. Praesent lacinia libero vel euismod consectetur.

\(\abs{\hat{x}_k}\)（\(k=0,\dots,N-1\)）は\cref{figure:frequency_domain}のようになる．
