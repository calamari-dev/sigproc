...
同様に計算すれば，\(0\leq m<N\)のとき\(\lagop^m\)の表現行列\(\mat{L}^m\)は
\[
...
\cref{equation:discrete_impulse_response}から\(H=\sum_{m=0}^{N-1}h_m\lagop^m\)なので，\(H\)の表現行列\(\mat{H}\)は
\begin{align*}
  \mat{H} &= h_0\matrice*{\trps{\vect{e}_1} \\ \trps{\vect{e}_2} \\ \vdots \\ \trps{\vect{e}_{N-1}} \\ \trps{\vect{e}_N}}+h_1\matrice*{\trps{\vect{e}_N} \\ \trps{\vect{e}_1} \\ \trps{\vect{e}_2} \\ \vdots \\ \trps{\vect{e}_{N-1}}}+h_2\matrice*{\trps{\vect{e}_{N-1}} \\ \trps{\vect{e}_N} \\ \trps{\vect{e}_1} \\ \vdots \\ \trps{\vect{e}_{N-2}}}+\dots+h_{N-1}\matrice*{\trps{\vect{e}_2} \\ \trps{\vect{e}_3} \\ \vdots \\ \trps{\vect{e}_N} \\ \trps{\vect{e}_1}} \\
...
\begin{proof}
  ...
  \((\dft{N}H\dft{N}^{-1})x=\dft{N}(h\circconv\dft{N}^{-1}x)=\hat{h}\cdot x=\hat{H}(x)\)だから，\(\hat{H}\)の表現行列\(\mat{\hat{H}}=\diag(\hat{h}[0],\dots,\hat{h}[N-1])\)は\(\dft{N}H\dft{N}^{-1}\)の表現行列\(\mat{W}\mat{H}\mat{W}^{-1}\)に一致する．
\end{proof}
