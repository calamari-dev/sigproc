...
明らかに\(\cycles{\numset{C}}{N}\)は数列空間の部分空間である．\(\cycles{\numset{C}}{N}\)上の\termdef{ラグ作用素}\index{らぐさようそ@ラグ作用素}（lag operator）\(\lagop\)を\(\lagop x=\seq{x[n-1]}\)で定義し，数列\(\shah\in\cycles{\numset{C}}{N}\)を
\[
  \shah[n] = \begin{cases}1 & \text{(\(n\)は\(N\)で割り切れる)},\\ 0 & \text{(otherwise)}\end{cases}
\]
と定める\indexsymbol{\(\shah\)}\footnote{\(\shah\)（sha）はキリル文字．}．

\begin{proposition}{}{}
  集合\(\basis{D}=\Set{\lagop^m\shah\given 0\leq m<N}\)は\(\cycles{\numset{C}}{N}\)の基底である．ただし\(\lagop^0x=x\)とする．
\end{proposition}

\begin{proof}
  \(x\in\cycles{\numset{C}}{N}\)，\(m,n\in\Set{0,\dots,N-1}\)を任意にとる．\(\abs{n-m}<N\)なので，\(n-m\)が\(N\)で割り切れるのは\(n-m=0\)のときだけである．
  よって\(x[m](\lagop^m\shah)[n]=x[m]\shah[n-m]=x[n]\kdelta{m}{n}\)だから
  \begin{equation}
    \label{equation:lagop_expansion}
    \sum_{m=0}^{N-1}x[m](\lagop^m\shah)[n] = x[n]\sum_{m=0}^{N-1}\kdelta{m}{n}
    = x[n]
  \end{equation}
  である．よって\(x=\sum_{m=0}^{N-1}x[m]\lagop^m\shah\)なので，\(\basis{D}\)は基底である．
\end{proof}

\begin{figure}[htbp]
  \centering
  \includegraphics{figures/shah.pdf}
  \caption{\(N=5\)のときの\(\shah\)と\(\lagop\shah\)の様子．}
\end{figure}
...
\begin{note}
  \cref{equation:lagop_expansion,equation:discrete_impulse_response}より，多項式\(X(T)=\sum x[m]T^m\)，\(Y(T)=\sum y[m]T^m\)に関して\(x\circconv y=X(\lagop)Y(\lagop)\shah\)が成立する．実は，\(X(T)Y(T)\)を\(T^N-1\)で割ったあまりは\(\sum(x\circconv y)[m]T^m\)である．
...
\begin{proof}
  \(\vect{x}=\trps{\rowvect{x_0 & \cdots & x_{N-1}}}\)，\(s=\sum_{m=0}^{N-1}x_m\lagop^m\shah\)とおくと，
...
\begin{figure}[htbp]
  \begin{minipage}{\linewidth/2}
    \centering
    \includegraphics{figures/stationary.pdf}
    \caption{定常信号の例．}
  \end{minipage}%
  \begin{minipage}{\linewidth/2}
    \centering
    \includegraphics{figures/non_stationary.pdf}
    \caption{非定常信号の例．}
  \end{minipage}
\end{figure}

\begin{definition}{短時間フーリエ変換}{discrete_stft}\index{たんじかんふーりえへんかん@短時間フーリエ変換}\index{まどかんすう@窓関数}\index{STFT|see{短時間フーリエ変換}}\index{Vw@\(\stft{w}\)}
  ...
  また，\(w\)を\(\stft[L]{w}\)の\termdef{窓関数}（window function）という．
\end{definition}
...
\begin{definition}{離散ガボール変換}{discrete_gabor}\index{りさんがぼーるへんかん@離散ガボール変換}\index{DGT|see{離散ガボール変換}}\index{Gw@\(\dgt{w}\)}
  ...
  \end{align*}
  に写す線型写像\(\dgt[K,N,L]{w}\colon\mapset{\numset{Z}}{\numset{C}}\to\mapset{\numset{Z}\times\numset{Z}}{\numset{C}}\)を\termdef{離散ガボール変換}（discrete Gabor transform; DGT）という．
\end{definition}
...
\begin{theorem}{Donoho‐Starkの不確定性原理}{discrete_donoho_stark}\index{Donoho‐Starkの不確定性原理}\index{ふかくていせいげんり@不確定性原理!Donoho‐Stark}
  ...
  \]
  である．これを\termdef{Donoho‐Starkの不確定性原理}（Donoho‐Stark uncertainity principle）という．
\end{theorem}
