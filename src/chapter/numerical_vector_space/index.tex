...
\]
で近似しよう．標本化周波数\(1/N\)で\(f\)を標本化すると，離散時間信号\(x[n]=f(t_n)\)（\(t_n=2\krez n/N\)）が得られる\footnote{信号処理では慣習的に，時刻を表す添字を大かっこで書く．}．

\(n=0,\dots,N-1\)における\(f(t_n)=x[n]\)の値を用いて\(f\)を\(f(\vect{c};t)\)にあてはめよう．
\(K\)の値を\(N\)より大きくすると，関数\(\phi_0,\dots,\phi_{K-1}\)の中に標本化後は区別できなくなる組が現れてしまう（たとえば\(\phi_0\)と\(\phi_N\)）．そこで\(K=N\)とし，残差平方和
\[
