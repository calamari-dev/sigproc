...
\(w\)をハン窓にして，\(M\)の値を変えたときの\(\abs{(\stft{w_M}x)[k,n]}^2\)の様子が\cref{figure:sharp_time}と\cref{figure:sharp_frequency}である．
...
明瞭さは\(w(t)\)の選択によっても変化する．\cref{figure:rectangular}と\cref{figure:hann}は，それぞれ\(w(t)\)が矩形窓とハン窓のときの\(\abs{(\stft{w_M}x)[k,n]}^2\)の様子である．ただし，\(M\)の値は256に設定した．
