...
\begin{definition}{巡回畳み込み}{}\index{じゅんかいたたみこみ@巡回畳み込み}\index{たたみこみ@畳み込み!じゅんかい@巡回}\indexsymbol{\(\circconv\)}
...
\begin{proof}
  2つめのみ示す．\cref{proposition:dft_convolution_theorem}より\(\dft{N}((x\circconv y)\circconv z)=\sqrt{N}(\dft{N}(x\circconv y))\cdot\hat{z}=N(\hat{x}\cdot\hat{y})\cdot\hat{z}\)である．
  よって\((x\circconv y)\circconv z=N\dft{N}^{-1}(\hat{x}\cdot\hat{y}\cdot\hat{z})\)であり，右辺は\(x\circconv (y\circconv z)\)とも等しい．
\end{proof}

最後に，\(\cycles{\numset{C}}{N}\)上で得られた結果を\(\numset{C}^N\)上のものに変換する．\(i\)を\(N\)で割ったあまりを\(\bar{\imath}\)と書く．
