...
そこで，\(T\)と\(Y\)の値をくり返し測定し，得られたデータを式\(y=at^2\)にあてはめれば，\(g=2a\)の値を間接的に測定できる．

以下では観測値の系列\(\trps{\rowvect{x_1 & y_1}},\dots,\trps{\rowvect{x_n & y_n}}\)を\(\trps{\rowvect{X & Y}}\)の標本と呼ぶ．
...
\begin{proof}
  まず\(P(x)\)が存在することを示す．
  \[
    L_k(x) = \frac{x-x_1}{x_k-x_1}\frac{x-x_2}{x_k-x_2}\dotsm\frac{x-x_{k-1}}{x_k-x_{k-1}}\frac{x-x_{k+1}}{x_k-x_{k+1}}\dotsm\frac{x-x_{p+1}}{x_k-x_{p+1}}
  \]
  とおくと，\(L_k(x_i)=\kdelta{i}{k}\)だから\(P(x)=\sum_{k=1}^{p+1}y_kL_k(x)\)は条件を満たす．

  次に\(P(x)\)が一意であることを示す．多項式\(P_1(x)\)と\(P_2(x)\)はどちらも条件を満たすとする．
  このとき，多項式\(D(x)=P_1(x)-P_2(x)\)は\(D(x_i)=y_i-y_i=0\)を満たすので，\(D(x)=Q(x)(x-x_1)\dotsm(x-x_{p+1})\)と因数分解できる．
  \(D(x)\)の次数は最大でも\(p\)だから\(Q(x)=0\)，すなわち\(P_1(x)=P_2(x)\)である．
\end{proof}

\footnotetext{この多項式を\termdef{補間多項式}（interpolating polynomial）という．}
実験で得られるデータは誤差を含むものだから，普通は標本を\(p+1\)個より多く測定し，平均的にもっともあてはまりがよい\(c_0,\dots,c_p\)の組を求める．
