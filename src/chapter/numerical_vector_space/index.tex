...
\begin{theorem}{スペクトル定理}{spectral_theorem}\index{すぺくとるていり@スペクトル定理}
  ...
  \begin{enumerate}
    \item \(\mat{A}\)は正規行列である．
    \item \(\mat{A}\)はユニタリ行列で対角化できる．すなわち，行列\(\mat{\Lambda}=\htrps{\mat{U}}\mat{A}\mat{U}\)が対角行列になるユニタリ行列\(\mat{U}\)が存在する．
  \end{enumerate}
\end{theorem}
...
\nameref{theorem:spectral_theorem}を証明する前に，次の定理を示そう．
...
\begin{proof}
  ...
  帰納法の仮定により，\(\mat{C}\)はシューア分解できる．\(\mat{C}\)のシューア分解\(\mat{C}=\mat{V}\mat{C}'\htrps{\mat{V}}\)から，\(n\)次ユニタリ行列\(\mat{W}\)を\(\mat{W}=\smallmatrice{\imat_d & \\ & \mat{V}}\)で定義する．このとき
  \[
    \matrice*{\lambda\imat_d & \mat{B} \\ & \mat{C}} = \matrice*{\lambda\imat_d & \mat{B} \\ & \mat{V}\mat{C}'\htrps{\mat{V}}}
    = \matrice*{\imat_d & \\ & \mat{V}}\matrice*{\lambda\imat_d & \htrps{\mat{V}}\mat{B}\mat{V} \\ & \mat{C}'}\matrice*{\imat_d & \\ & \htrps{\mat{V}}}
  \]
  だから，\(\mat{A}'=\smallmatrice{\lambda\imat_d & \htrps{\mat{V}}\mat{B}\mat{V} \\ & \mat{C}'}\)とおくと\(\htrps{\mat{U}}\mat{A}\mat{U}=\mat{W}\mat{A}'\htrps{\mat{W}}\)である．
  よって，\(\mat{U}\mat{W}\)を\(\mat{U}\)と置き直せば\(\mat{A}=\mat{U}\mat{A}'\htrps{\mat{U}}\)で，\(\mat{A}'\)は上三角行列なので，これは\(\mat{A}\)のシューア分解になっている．
\end{proof}

\cref{theorem:schur_decomposition}を利用すると，\nameref{theorem:spectral_theorem}は次のようにして示せる．

\begin{proof}[\nameref{theorem:spectral_theorem}の証明]
