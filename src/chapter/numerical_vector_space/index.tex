...
\begin{proof}
  ...
  \[
    \argmin_{\vect{x}\in\numset{K}^p}\vnorm{\mat{A}\vect{x}-\vect{b}} = \inv*{T}{\argmin_{\vect{z}\in\img T}\vnorm{\vect{z}-\vect{b}}}
    = \inv{T}{\Set{\proj_{\img T}\vect{b}}}
  \]
  が成立する．よって\(\argmin_{\vect{x}\in\numset{K}^p}\vnorm{\vect{\epsilon}(\vect{x})}=\inv{T}{\Set{\proj_{\img T}\vect{b}}}\)であり，この集合は明らかに空でない．
\end{proof}
...
これまでと同様，\(\mat{A}\)は任意の\(n\times p\)複素行列とする．まず，\(\pinv{\mat{A}}\)が\(\mat{A}\)に対して一意に定まることを示そう．
...
\begin{proposition}{}{}
  \(\mat{A}\)は\(n\times p\)複素行列で，\(\vect{b}\in\numset{C}^n\)とする．
  このとき，\(\pinv{\mat{A}}\vect{b}\)は\(S=\argmin_{\vect{x}\in\numset{C}^p}\vnorm{\mat{A}\vect{x}-\vect{b}}\)の元でノルムが最も小さく，同じノルムを持つ元はほかにない．
  すなわち\(\Set{\pinv{\mat{A}}\vect{b}}=\argmin_{\vect{x}\in S}\vnorm{\vect{x}}\)である．
\end{proposition}

\begin{proof}
  \cref{proposition:pseudoinverse_characterization}から\(\pinv{(\pinv{\mat{A}})}=\mat{A}\)はすぐ分かる．
  \cref{proposition:pseudoinverse_least_square}より\(S\)の任意の元は\(\vect{x}=\pinv{\mat{A}}\vect{b}+(\imat-\mat{A}\pinv{\mat{A}})\vect{w}\)と書けて，
  \cref{proposition:pseudoinverse_projection}から\((\imat-\mat{A}\pinv{\mat{A}})\vect{w}=\proj_{\pcomp{V}}\vect{w}\)（\(V=\colsp\pinv{\mat{A}}\)）なので\(\vnorm{\vect{x}}^2=\vnorm{\pinv{\mat{A}}\vect{b}}^2+\vnorm{\proj_{\pcomp{V}}\vect{w}}^2\)である．
  よって\(\Set{\pinv{\mat{A}}\vect{b}}=\argmin_{\vect{x}\in S}\vnorm{\vect{x}}\)である．
\end{proof}
