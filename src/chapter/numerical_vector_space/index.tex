...
\cref{section:stft,section:discrete_mra}では，時とともに様子が変わる旋律・地震動・景気のような信号を調べるのに欠かせない，\termdef{時間周波数解析}\index{じかんしゅうはすうかいせき@時間周波数解析}（time–frequency analysis）の理論を説明する．
...
\begin{proof}
  ...
  すると\(\innerp{\vect{x}}{\vect{\phi}_i}=0\)なので，いかなる\(A>0\)についても\(\sum_{i\in I}\abs{\innerp{\vect{x}}{\vect{\phi}_i}}^2=0<A\vnorm{\vect{x}}^2\)となる．よって\(\seq{\vect{\phi}_i}_{i\in I}\)はフレームでない．
\end{proof}
...
\begin{example}
  ...
  \(\seq{\vect{\phi}_k}_{k=0}^2\)を\termdef{メルセデス・ベンツフレーム}\index{めるせですべんつふれーむ@メルセデス・ベンツフレーム}\index{ふれーむ@フレーム!めるせですべんつ@メルセデス・ベンツ\texttwoemdash}（Mercedes‐Benz frame）という\cite{casazza2013}．名称の由来は図から明らかであろう．
\end{example}
...
\cref{definition:frame_analysis_synthesis}は以前に見た，正規直交基底に関する\(T\)と\(\synth{T}\)の拡張になっている\footnote{というより，分析（合成）作用素はフレームに対して定義するのが普通である．}．
ただし，フレームが正規直交基底でないときは\(T^{-1}=\synth{T}\)とも\(S=\synth{T}T\)が恒等変換であるとも限らない．

基底でない生成系を\termdef{\ltjruby{冗長}{じようちよう}}\index{じょうちょう@冗長}（redundant）な生成系という．フレームを考える動機の1つは，正規直交基底による分析・合成を冗長な生成系に拡張することである．
...
\begin{proposition}{}{stft_tight_frame}
  列\(\seq{\modulate{k}\translate{n}w}_{(k,n)\in J^2}\)は\(\cycles{L}\)のタイトフレームである．
\end{proposition}
...
\begin{lemma}{}{grammian_equals_zero}
  \(G\)は\(H\)上の線形変換で，任意の\(\vect{x},\vect{y}\in H\)に対し\(\innerp{\vect{x}}{G(\vect{y})}=\innerp{G(\vect{x})}{\vect{y}}\)，\(\innerp{\vect{x}}{G(\vect{x})}=0\)であるとする．
  このとき\(\img G=\Set{\zvec}\)である．
\end{lemma}

\begin{proof}
  任意の\(\vect{x},\vect{y}\in H\)に対して\(\innerp{\vect{x}+\vect{y}}{G(\vect{x}+\vect{y})}=\innerp{\vect{x}}{G(\vect{y})}+\innerp{\vect{y}}{G(\vect{x})}=\innerp{G(\vect{x})}{\vect{y}}+\conj*{\innerp{G(\vect{x})}{\vect{y}}}=0\)だから，
  \(\vect{y}=G(\vect{x})\)とすると\(2\vnorm{G(\vect{x})}^2=0\)が得られる．よって\(G(\vect{x})=\zvec\)，\(\img G=\Set{\zvec}\)である．
\end{proof}

\begin{proposition}{タイトフレームの逆変換}{tight_frame_inversion}
  列\(\Phi=\seq{\vect{\phi}_i}_{i\in I}\)を最良フレーム限界\(A\)のタイトフレームとする．このとき，\(\Phi\)のフレーム作用素\(S\)は恒等変換の\(A\)倍である．すなわち
  \[
    \vect{x} = \frac{1}{A}S(\vect{x})
    = \frac{1}{A}\sum_{i\in I}\innerp{\vect{x}}{\vect{\phi}_i}\vect{\phi}_i\quad(\vect{x}\in H)
  \]
  が成立する．
\end{proposition}

\begin{proof}
  線形変換\(G(\vect{x})=\vect{x}-(1/A)S(\vect{x})\)について\(\img G=\Set{\zvec}\)を示せばよい．
  \[
    \innerp{G(\vect{x})}{\vect{y}} = \innerp{\vect{x}}{\vect{y}}-\frac{1}{A}\sum_{i\in I}\innerp{\vect{x}}{\vect{\phi}_i}\innerp{\vect{\phi}_i}{\vect{y}}
    = \innerp{\vect{x}}{\vect{y}}-\frac{1}{A}\sum_{i\in I}\conj*{\innerp{\vect{y}}{\vect{\phi}_i}}\innerp{\vect{x}}{\vect{\phi}_i}
  \]
  であり，右辺は\(\innerp{\vect{x}}{G(\vect{y})}\)に等しい．また，\(\vect{y}=\vect{x}\)なら\(\innerp{G(\vect{x})}{\vect{x}}=\vnorm{\vect{x}}^2-(1/A)A\vnorm{\vect{x}}^2=0\)である．
  よって\cref{lemma:grammian_equals_zero}より\(\img G=\Set{\zvec}\)である．
\end{proof}

\begin{corollary}{STFTの逆変換}{}
  \(\hat{x}=\stft{w}x\)（\(x\in\cycles{L}\)）とする．このとき
  \begin{align*}
    x[n] &= \frac{1}{L\vnorm{w}^2}\sum_{k,m\in J^2}\hat{x}[k,m](\modulate{k}\translate{m}w)[n] \\
    &= \frac{1}{L\vnorm{w}^2}\sum_{k=0}^{L-1}\sum_{m=0}^{L-1}\hat{x}[k,m]w[n-m]\napr^{2\krez\iuni kn/L}
  \end{align*}
  である．
\end{corollary}
