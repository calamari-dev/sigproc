...
\(\phi\)と同様，任意の\(\vect{y}\in W\)に対して
\[
  \phi(\vect{y}) = \trps{\begin{bmatrix}d_1 & \cdots & d_n\end{bmatrix}}
  \iff\vect{y} = d_1\vect{w}_1+\dots+d_n\vect{w}_n
\]
を満たす写像\(\psi\colon W\to\numset{K}^n\)が定義できる．
...
が定まり，\(\mat{A}\)は\cref{equation:representation_matrix}を満たす．この\(\mat{A}\)を\(f\)の\termdef{表現行列}という．

\begin{definition}{表現行列}{representation_matrix}\index{ひょうげんぎょうれつ@表現行列}
準備中．
\end{definition}
