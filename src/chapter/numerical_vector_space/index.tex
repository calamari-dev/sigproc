...
\begin{proof}
  ...
  \[
    \argmin_{\vect{y}\in V}\vnorm{\vect{x}-\vect{y}} = \Set*{z_1\vect{e}_1+\dots+z_m\vect{e}_m\given\trps{\begin{bmatrix}z_1 & \cdots & z_m\end{bmatrix}}\in\argmin_{\vect{z}\in\numset{C}^m}f(\vect{z})}
  \]
  である．
...
\begin{proof}
  ...
  \(\vect{n}\)の定義から，関数\(f(z)=\vnorm{\vect{x}-(\vect{n}+z\vect{v})}^2\)（\(z\in\numset{C}\)）は\(\argmin_{z\in\numset{C}}f(z)=\Set{0}\)を満たす．一方，\(x=\rpart z\)，\(y=\ipart z\)とおくと
  \begin{align*}
    f(z) &= \vnorm{\vect{x}-\vect{n}-z\vect{v}}^2
    = \vnorm{\vect{x}-\vect{n}}^2-2\rpart[(x-\iuni y)\innerp{\vect{x}-\vect{n}}{\vect{v}}]+\abs{z}^2\vnorm{\vect{v}}^2 \\
    &= \vnorm{\vect{x}-\vect{n}}^2-2(x\rpart\innerp{\vect{x}-\vect{n}}{\vect{v}}+y\ipart\innerp{\vect{x}-\vect{n}}{\vect{v}})+x^2+y^2 \\
    &= \vnorm{\vect{x}-\vect{n}}^2+(x-\rpart\innerp{\vect{x}-\vect{n}}{\vect{v}})^2+(y-\ipart\innerp{\vect{x}-\vect{n}}{\vect{v}})^2-\abs{\innerp{\vect{x}-\vect{n}}{\vect{v}}}^2 \\
    &= \vnorm{\vect{x}-\vect{n}}^2+\abs{z-\innerp{\vect{x}-\vect{n}}{\vect{v}}}^2-\abs{\innerp{\vect{x}-\vect{n}}{\vect{v}}}^2
  \end{align*}
  なので\(\argmin_{z\in\numset{C}}f(z)=\Set{\innerp{\vect{x}-\vect{n}}{\vect{v}}}\)，よって\(\innerp{\vect{x}-\vect{n}}{\vect{v}}=0\)である．
\end{proof}
