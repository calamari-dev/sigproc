...
たとえば，CDでは音声信号の瞬時値を1秒あたり44100個記録している．すなわち，時刻\(t\)秒における瞬時値を\(x(t)\)，収録時間を\(T\)秒とおくと，CDには数列\(\seq{x(n/44100)}_{n=0}^T\)が記録されている．
...
\(\vect{x}\)を離散フーリエ変換すると，\(\abs{\hat{x}_j}\)（\(j=0,\dots,n-1\)）は\cref{figure:frequency_domain}のようになる．
...
\begin{definition}{多次元離散フーリエ変換}{multidimensional_discrete_fourier_transform}\index{りさんふーりえへんかん@離散フーリエ変換!たじげん@多次元}\index{FZdn@\(\dft[d]{\vect{n}}x\)}
  ...
  関数\(x\colon\Omega\to\numset{C}\)に対して，関数
  \[
    \hat{x}(\vect{\nu}) = \frac{1}{\sqrt{\det\mat{N}}}\sum_{\vect{r}\in\Omega}x(\vect{r})\napr^{-2\krez\iuni\trps{\vect{\nu}}\mat{N}^{-1}\vect{r}}\quad(\vect{\nu}\in\Omega)
  \]
