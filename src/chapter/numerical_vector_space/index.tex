...
\begin{proof}
  ...
  \begin{align*}
    ...
    &= \vnorm{\vect{x}}^2+\sum(\abs{z_i}^2-2\rpart(\conj{z}_i\innerp{\vect{x}}{\vect{e}_i})) \\
    &= \vnorm{\vect{x}}^2+\sum(s_i^2+t_i^2-2\rpart((s_i-\iuni t_i)\innerp{\vect{x}}{\vect{e}_i})) \\
...
主成分分析の目的は，この問題を解決するために\(\vect{x}\)の特徴を保ったまま次元を減らす\index{じげんさくげん@次元削減}\footnote{次元を減らす手法を一般に\termdef{次元削減}（dimensionality reduction）という．}ことである．
