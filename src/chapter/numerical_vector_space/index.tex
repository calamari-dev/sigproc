...
\begin{definition}{離散フーリエ変換}{discrete_fourier_transform}\index{りさんふーりえへんかん@離散フーリエ変換!かずべくとる@数ベクトル}\index{DFT|see{離散フーリエ変換}}\index{FZN@\(\dft{N}\)}
  ...
  \]
  で定義される\(\numset{C}^N\)上の線型変換\(\dft{N}\)を\termdef{離散フーリエ変換}（discrete Fourier transform; DFT）という．
\end{definition}
...
\begin{definition}{}{cycle_dft}\index{りさんふーりえへんかん@離散フーリエ変換!しゅうきすうれつ@周期数列}\index{l2N@\(\cycles{N}\)}\index{FZN@\(\dft{N}\)}
  \(N\)を周期に持つ複素数列全体を\(\cycles{N}\)と書く．\(\cycles{N}\)上の線型変換\(\dft{N}\)を次式で定義する．
  \[
...
\(h\in\cycles{N}\)を任意にとり，\(\cycles{N}\)上の線型変換\(H\)を\(H(x)=h\circconv x\)で定義する．
...
\begin{proof}
  ...
  \(\cycles{N}\)上の線型写像\(H\)を\(H(x)=h\circconv x\)で定義すると，\cref{proposition:dft_convolution_theorem}より\((\dft{N}H\dft*{N})x=\dft{N}(h\circconv\dft*{N}x)=\hat{h}\cdot x\)となる．よって，\(\dft{N}H\dft*{N}\)の表現行列は\(\diag(\hat{h}[0],\dots,\hat{h}[N-1])\)である．
...
\begin{proposition}{}{stft_and_shift}
  \(\cycles{L}\)上の線型変換\(\translate{m}\)，\(\modulate{k}\)を
  \[
...
\begin{corollary}{}{stft_cyclic}
  \(\stft{w}\)は\(\img\stft{w}\subseteq\doublecycles{L}{L}\)を満たす線型写像である．
\end{corollary}

\begin{proof}
  線型性は\cref{proposition:stft_and_shift}からただちにしたがう．\(x\in\cycles{L}\)と\(k,n\in\numset{Z}\)を任意にとる．
...
\begin{figure}[htbp]
  \centering
  \includegraphics{figures/window.pdf}
...
\]
より\(\abs{\dft{L}(x\cdot\translate{n}\conj{w})[k]}^2\)の定数倍である．また\cref{figure:window}のように，\(\translate{n}\conj{w}\)を掛けると\(x\)から時刻\(n\)の周りだけが切り出される．
そのため，\cref{figure:phone_spectrogram}は「各時刻\(n\)の周りにおける周波数成分のグラフ」と解釈できる．
2重数列\(\seq{\abs{(\stft{w}x)[k,n]}^2}\)と\ltjkenten{その}グラフを\(x\)の\termdef{スペクトログラム}\index{すぺくとろぐらむ@スペクトログラム}（spectrogram）といい，波形からは読み取りがたい情報を多く含んでいる．

例として，\cref{figure:phone_spectrogram}中で縦に積み重なっている白い横線に注目する．このうち最も下にある横線は，基本周波数（\cref{subsection:dft}参照）が時間変化してできた軌跡である．したがって，この軌跡をなぞれば旋律の時間変化を読み取れる．
...
\begin{definition}{離散ガボール変換}{discrete_gabor}\index{りさんがぼーるへんかん@離散ガボール変換}\index{DGT|see{離散ガボール変換}}\index{Gw@\(\dgt{w}\)}
  ...
  \end{align*}
  に写す線型写像\(\dgt[K,N,L]{w}\colon\cycles{L}\to\doublecycles{L/K}{L/N}\)を\termdef{離散ガボール変換}（discrete Gabor transform; DGT）という．
\end{definition}
