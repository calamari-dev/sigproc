...
また，\(\increment\vect{x}=-t\vect{g}\)（\(t>0\)，\(\vect{g}=\htrps{\mat{A}}\mat{A}\vect{x}_0-\htrps{\mat{A}}\vect{y}\)）のとき\(\increment\epsilon(\vect{x}_0)=-2t\vnorm{\vect{g}}^2+t^2\vnorm{\mat{A}\vect{g}}^2\)なので，
\(\vect{g}=\zvec\)でなければ\(t\)の値が十分小さいとき\(\increment\epsilon(\vect{x}_0)<0\)である．つまり，\(\htrps{\mat{A}}\mat{A}\vect{x}_0=\htrps{\mat{A}}\vect{y}\)でなければ\(\epsilon(\vect{x}_0)\)は関数\(\epsilon\)の最小値ではない．
...
\begin{note}
  ...
  \(\increment\vect{x}\)を無限小とみなせば\(\intd*{\epsilon}=2\vect{g}\cdot\intd*{\vect{x}}\)である．つまり，\(2\vect{g}=2(\htrps{\mat{A}}\mat{A}\vect{x}-\htrps{\mat{A}}\vect{y})\)は\(\epsilon(\vect{x})\)の勾配である．
\end{note}
