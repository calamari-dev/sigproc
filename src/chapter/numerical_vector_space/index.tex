...
\begin{definition}{短時間フーリエ変換}{discrete_stft}\index{たんじかんふーりえへんかん@短時間フーリエ変換}\index{STFT|see{短時間フーリエ変換}}\index{Vw@\(\stft{w}\)}
  ...
  \]
  に写す写像\(\stft[L]{w}\colon\cycles{\numset{C}}{L}\to\cycles[2]{\numset{C}}{L,L}\)を\termdef{短時間フーリエ変換}（short‐time Fourier transform; STFT）という．
\end{definition}

\(w\)を\(\stft[L]{w}\)の\termdef{窓関数}\index{まどかんすう@窓関数}（window function）という．文脈から\(L\)の値が明らかであれば，\(\stft[L]{w}\)を単に\(\stft{w}\)と書く．

\(x,y\in\cycles{\numset{C}}{L}\)の内積を\(\innerp{x}{y}=\sum_{n=0}^{L-1}x[n]\conj*{y[n]}\)で定義する．

\begin{proposition}{}{stft_and_shift}
  \(\cycles{\numset{C}}{L}\)上の写像\(\tshift{m}\)，\(\fshift[L]{k}\)を
  \[
    (\tshift{m}x)[n] = x[n-m],
    \quad(\fshift[L]{k}x)[n] = \napr^{2\krez\iuni kn/L}x[n]
  \]
  で定義する．このとき，任意の\(w,x\in\cycles{\numset{C}}{L}\)に対して次式が成立する．
  \[
    (\stft[L]{w}x)[k,n] = \innerp{x}{\fshift[L]{k}\tshift{n}w}\quad(k,n\in\numset{Z})
  \]
\end{proposition}
...
\begin{proof}
  実際
  \begin{align*}
    \innerp{x}{\fshift[L]{k}\tshift{n}w} &= \sum_{m=0}^{L-1}x[m]\conj*{\napr^{2\krez\iuni km/L}(\tshift{n}w)[m]} \\
    &= \sum_{m=0}^{L-1}x[m]\napr^{-2\krez\iuni km/L}\conj*{w[m-n]}
    = (\stft[L]{w})[k,n]
  \end{align*}
  である．
\end{proof}
