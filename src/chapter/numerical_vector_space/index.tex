...
\begin{definition}{フレーム}{frame}\index{ふれーむ@フレーム}
  \(\seq{\vect{e}_i}_{i\in I}\)を\(H\)上の有限列とする．集合\(\Set{\vect{e}_i\given i\in I}\)が\(H\)の生成系であるとき，\(\seq{\vect{e}_i}_{i\in I}\)を\(H\)の\termdef{フレーム}（frame）という．
\end{definition}

\begin{proposition}{}{frame_characterization}
  \(\seq{\vect{e}_i}_{i\in I}\)を\(H\)のフレームとする．このとき，ある実数\(A,B>0\)が存在し，任意の\(\vect{v}\in H\)に対して次式を満たす．
  \[
...
\cref{proposition:frame_characterization}を示すにあたり，1つ注意すべきことがある．我々は\cref{section:finite_projection}で，直交射影と直交補空間を\(\numset{K}^n\)の部分空間に対して定義した．
しかし，\cref{section:finite_projection}の議論は\(\numset{K}^n\)を\(H\)に置き換えてもなんら問題ない．そのため，\(H\)の部分空間\(V\)についても\(\proj_V\)と\(\pcomp{V}=\pcomp[H]{V}\)はまったく同様に定義される．
...
\begin{note}
  実は\cref{proposition:frame_characterization}の逆も成り立つ．つまり，フレーム限界を持つ列は必ずフレームである（\cite{casazza2013}に証明がある）．
  証明には，\(\dim H<+\infty\)より\(H\)の単位球面はコンパクトであり，単位球面上の点列はすべて収束部分列を持つことが効いてくる．
\end{note}
...
\begin{example}[\(\cycles{N}\)上のDFT]
  ...
  \]
  だから，\(\seq{w_k}_{k=0}^{N-1}\)はフレーム限界\(A=B=1\)のフレームである．
\end{example}
