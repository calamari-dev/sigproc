...
\begin{figure}[htbp]
  ...
  \end{minipage}
  \caption{画像とその2次元離散フーリエ変換．}
  \label{figure:pudding_dft}
\end{figure}

\cref{figure:pudding_dft}は，モノクロ画像を離散信号\(x=x[r_1,r_2]\)とみなし\(\abs{\dft[2]{\vect{n}}x}\)を計算した様子である．
ただし\(\vect{n}\)は，この画像が\(1024\times 1024\)の画素からなるので\(\vect{n}=\trps{\rowvect{1024 & 1024}}\)である．
...
\begin{figure}[htbp]
  ...
  \end{minipage}
  \caption{\(\cos(3x+4y)\)（\(x,y\in\Set{2\krez(k-32)/64\given 0\leq k<64}\)）のグラフと2次元離散フーリエ変換．}
\end{figure}
