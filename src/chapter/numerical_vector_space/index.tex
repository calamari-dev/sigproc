...
\begin{note}
  ...
  \]
  である．よって，正規方程式は\(\diffp*{\epsilon(\vect{x})}/{x_i}=0\)（\(1\leq i\leq p\)）と同値である．
\end{note}
...
\subsection{スペクトル分解の証明}

\begin{theorem}{シューア分解}{}\index{しゅーあぶんかい@シューア分解}
  任意の複素正方行列はユニタリ行列で上三角化できる．すなわち，任意の\(n\)次複素正方行列\(\mat{A}\)に対し，
  \(n\)次ユニタリ行列\(\mat{U}\)が存在して，行列\(\tilde{\mat{A}}=\htrps{\mat{U}}\mat{A}\mat{U}\)は以下の形になる．
  式\(\mat{A}=\mat{U}\tilde{\mat{A}}\htrps{\mat{U}}\)を\(\mat{A}\)の\termdef{シューア分解}（Schur decomposition）という．
  \[
    \tilde{\mat{A}} = \matrice*{\midx{\tilde{a}}{1}{1} & \midx{\tilde{a}}{1}{2} & \cdots & \midx{\tilde{a}}{1}{n} \\ & \midx{\tilde{a}}{2}{2} & \cdots & \midx{\tilde{a}}{2}{n} \\ & & \ddots & \vdots \\ 0 & & & \midx{\tilde{a}}{n}{n}}
  \]
\end{theorem}

\begin{proof}
  \(n\)に関する帰納法で示す．\(E\)を\(\mat{A}\)のある固有値\(\lambda\)に属する固有空間とする．
  また，集合\(\basis{B}_1=\Set{\vect{u}_1,\dots,\vect{u}_d}\)，\(\basis{B}_2=\Set{\vect{u}_{d+1},\dots,\vect{u}_n}\)をそれぞれ\(E\)，\(\pcomp{E}\)の正規直交基底とする．
  このとき，集合\(\basis{B}_1\cup\basis{B}_2\)は\(\numset{C}^n\)の正規直交基底だから，行列\(\vect{U}=\rowvect{\vect{u}_1 & \cdots & \vect{u}_n}\)はユニタリ行列である．

  \(\numset{C}^n\)の標準基底を\(\Set{\vect{e}_1,\dots,\vect{e}_n}\)とおくと，\(\htrps{\mat{U}}\mat{A}\vect{u}_i=\lambda\htrps{\mat{U}}\vect{u}_i=\lambda\vect{e}_i\)（\(1\leq i\leq d\)）なので
  \(\htrps{\mat{U}}\mat{A}\mat{U}=\rowvect{\lambda\vect{e}_1 & \cdots & \lambda\vect{e}_d & \htrps{\mat{U}}\mat{A}\vect{u}_{d+1} & \cdots & \htrps{\mat{U}}\mat{A}\vect{u}_n}\)である．
  右辺を\(\smallmatrice{\lambda\imat_d & \mat{B} \\ & \mat{C}}\)とおくと，\(\mat{B}\)は\((n-d)\times d\)行列，\(\mat{C}\)は\((n-d)\times(n-d)\)行列である．

  帰納法の仮定により，\(\mat{C}\)はシューア分解できる．\(\mat{C}\)のシューア分解\(\mat{C}=\mat{V}\tilde{\mat{C}}\htrps{\mat{V}}\)から，\(n\)次ユニタリ行列\(\mat{W}\)を\(\mat{W}=\smallmatrice{\imat_d & \\ & \mat{V}}\)で定義する．このとき
  \[
    \matrice*{\lambda\imat_d & \mat{B} \\ & \mat{C}} = \matrice*{\lambda\imat_d & \mat{B} \\ & \mat{V}\tilde{\mat{C}}\htrps{\mat{V}}}
    = \matrice*{\imat_d & \\ & \mat{V}}\matrice*{\lambda\imat_d & \htrps{\mat{V}}\mat{B}\mat{V} \\ & \tilde{\mat{C}}}\matrice*{\imat_d & \\ & \htrps{\mat{V}}}
  \]
  だから，\(\tilde{\mat{A}}=\smallmatrice{\lambda\imat_d & \htrps{\mat{V}}\mat{B}\mat{V} \\ & \tilde{\mat{C}}}\)とおくと\(\htrps{\mat{U}}\mat{A}\mat{U}=\mat{W}\tilde{\mat{A}}\htrps{\mat{W}}\)である．
  よって，\(\mat{U}\mat{W}\)を\(\mat{U}\)と置き直せば\(\mat{A}=\mat{U}\tilde{\mat{A}}\htrps{\mat{U}}\)で，\(\tilde{\mat{A}}\)は上三角行列なので，これは\(\mat{A}\)のシューア分解になっている．
\end{proof}
