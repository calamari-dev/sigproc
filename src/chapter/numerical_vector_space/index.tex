...
\[
  \sum_{i=0}^{N-1}x_i\delta_i \longleftrightarrow \sum_{i=0}^{N-1}x_i\vect{e}_i
\]
と表すと，\(\dft{N}\)は\(\longleftrightarrow\)を保ち，次式が成立する（\cref{subsection:representation_matrix}を見よ）．
\[
  \dft{N}\pqty*{\sum_{i=0}^{N-1}x_i\delta_i} = \sum_{i=0}^{N-1}x_i\hat{\delta}_i
  \longleftrightarrow \sum_{i=0}^{N-1}x_i\vect{\hat{e}}_i
...
\begin{proposition}{}{dft_shift_theorem}
  任意の\(x\in\cycles{\numset{C}}{N}\)に対して\((\dft{N}\lagop x)[k]=\zeta^{-k}\hat{x}_k\)である．
\end{proposition}
...
\begin{proof}
  ...
  \]
  となる．右辺は周期数列の1周期に渡る和だから，\(\sum_{m=1}^N\)を\(\sum_{m=0}^{N-1}\)に変えても値は変わらない．よって\(\zeta^{-k}\hat{x}_k=(\dft{N}\lagop x)[k]\)である．
\end{proof}
...
\begin{proposition}{}{dft_convolution_theorem}
  ...
  \[
    \dft{N}(x\circconv y)[k] = \sqrt{N}\hat{x}[k]\hat{y}[k],
    \quad\dft{N}(x\cdot y)[k] = \sqrt{N}(\hat{x}\circconv\hat{y})[k]
  \]
\end{proposition}
...
\begin{proof}
  ...
  \end{align*}
  となる．\cref{proposition:dft_shift_theorem}より\((\dft{N}\lagop^my)[k]=\zeta^{-mk}\hat{y}_k\)なので
  \[
...
\begin{proof}
  2つめのみ示す．\(\dft{N}((x\circconv y)\circconv z)=\sqrt{N}(\dft{N}(x\circconv y))\cdot\hat{z}=N(\hat{x}\cdot\hat{y})\cdot\hat{z}\)より
...
\(h\in\cycles{\numset{C}}{N}\)を任意にとり，作用素\(\operatorname{\symit{H}}\colon\cycles{\numset{C}}{N}\to\cycles{\numset{C}}{N}\)を\(\operatorname{\symit{H}}x=x\circconv h\)で定義する．
基底\(\basis{D}\)に関する\(\operatorname{\symit{H}}\)の表現行列\(\mat{H}\)は
\[
  \mat{H} = \matrice*{h[0] & h[N-1] & h[N-2] & \cdots & h[2] & h[1] \\ h[1] & h[0] & h[N-1] & \cdots & h[3] & h[2] \\ h[2] & h[1] & h[0] & \cdots & h[4] & h[3] \\ \vdots & \vdots & \vdots & \ddots & \vdots \\ h[N-2] & h[N-3] & h[N-4] & \cdots & h[0] & h[N-1] \\  h[N-1] & h[N-2] & h[N-3] & \cdots & h[1] & h[0]}
\]
...
\begin{proposition}{}{}
  巡回行列は離散フーリエ変換で対角化される．すなわち，任意の\(N\)次巡回行列\(\mat{H}\)について\(\mat{W}^{-1}\mat{H}\mat{W}\)は対角行列である．
\end{proposition}

\begin{proof}
  \(\mat{H}\)の第1列を\(\trps{\rowvect{h_0 & \cdots h_{N-1}}}\)とおき，\(\cycles{\numset{C}}{N}\)上の作用素\(\operatorname{\symit{H}}\)，\(\operatorname{\symit{\hat{H}}}\)をそれぞれ
  \(\operatorname{\symit{H}}x=h\circconv x\)，\(\operatorname{\symit{\hat{H}}}x=\hat{h}\cdot x\)（\(h=\sum_{i=0}^{N-1}h_i\delta_i\)）で定義する．\cref{proposition:dft_convolution_theorem}より
  \[
    (\dft{N}^{-1}\operatorname{\symit{H}}\dft{N})x = \dft{N}^{-1}(\hat{h}\circconv\dft{N}x)
    = \hat{h}\cdot x
    = \operatorname{\symit{\hat{H}}}x\quad(x\in\cycles{\numset{C}}{N})
  \]
  だから，基底\(\basis{D}\)に関する\(\operatorname{\symit{\hat{H}}}\)の表現行列\(\mat{\hat{H}}=\diag(\hat{h}[0],\dots,\hat{h}[N-1])\)は，\(\dft{N}^{-1}\operatorname{\symit{H}}\dft{N}\)の表現行列\(\mat{W}^{-1}\mat{H}\mat{W}\)でもある．
\end{proof}
