...
\begin{proposition}{}{pseudoinverse_projection}
  \(\proj_{\colsp\mat{A}}\vect{x}=\mat{A}\pinv{\mat{A}}\vect{x}\)，\(\proj_{\pcomp{(\colsp\mat{A})}}\vect{x}=(\imat-\mat{A}\pinv{\mat{A}})\vect{x}\)である．
\end{proposition}

\begin{proof}
  \cref{proposition:pseudoinverse_characterization}より，各\(\vect{y}\in\numset{C}^p\)に対して
  \(\innerp{\mat{A}\pinv{\mat{A}}\vect{x}-\vect{x}}{\mat{A}\vect{y}}=\innerp{(\htrps{\mat{A}}\htrps{(\mat{A}\pinv{\mat{A}})}-\htrps{\mat{A}})\vect{x}}{\vect{y}}=\innerp{(\htrps{(\mat{A}\pinv{\mat{A}}\mat{A})}-\htrps{\mat{A}})\vect{x}}{\vect{y}}=\innerp{\zmat\vect{x}}{\vect{y}}=0\)
  である．よって\(\proj_{\colsp\mat{A}}\vect{x}=\mat{A}\pinv{\mat{A}}\vect{x}\)である．後半は\(\vect{x}=(\proj_{\colsp\mat{A}}\vect{x})+(\proj_{\pcomp{(\colsp\mat{A})}}\vect{x})\)から分かる．
\end{proof}
...
\begin{proof}
  \cref{proposition:pseudoinverse_least_square}より\(S\)の任意の元は\(\vect{x}=\pinv{\mat{A}}\vect{b}+(\imat-\pinv{\mat{A}}\mat{A})\vect{w}\)と書けて，
  \cref{proposition:pseudoinverse_projection}から\((\imat-\pinv{\mat{A}}\mat{A})\vect{w}=(\imat-\mat{X}\pinv{\mat{X}})\vect{w}=\proj_{\pcomp{(\colsp\mat{X})}}\vect{w}\)（\(\mat{X}=\pinv{\mat{A}}\)）である．
  よって\(\vnorm{\vect{x}}^2=\vnorm{\mat{X}\vect{b}}^2+\vnorm{\proj_{\pcomp{(\colsp\mat{X})}}\vect{w}}^2\)なので，\(\argmin_{\vect{x}\in S}\vnorm{\vect{x}}=\Set{\mat{X}\vect{b}}=\Set{\pinv{\mat{A}}\vect{b}}\)である．
\end{proof}
...
この方程式の解を1つ選び\(\tilde{\vect{x}}\)とおく．すると，他の解\(\vect{x}\)は\(\mat{A}(\vect{x}-\tilde{\vect{x}})=\tilde{\vect{b}}-\tilde{\vect{b}}=\zvec\)を満たす．
つまり，\(\nul\mat{A}=\Set{\vect{y}\in\numset{K}^p\given\mat{A}\vect{y}=\zvec}\)とおくと\(\vect{x}-\tilde{\vect{x}}\in\ker\mat{A}\)である．

逆に，\(\numset{K}^p\)の元\(\vect{x}\)が\(\vect{x}-\tilde{\vect{x}}\in\ker\mat{A}\)を満たせば\(\mat{A}\vect{x}=\tilde{\vect{b}}\)も成立する．
つまり\(\argmin_{\vect{x}\in\numset{K}^p}\vnorm{\mat{A}\vect{x}-\vect{b}}\)は，部分空間\(\ker\mat{A}\)を\(\tilde{\vect{x}}\)によって平行移動した集合\(\Set{\tilde{\vect{x}}+\vect{y}\given\vect{y}\in\ker\mat{A}}\)である．
このような集合をアフィン部分空間という．
...
任意の\(\vect{x}\in\numset{K}^p\)とアフィン部分空間\(S\subset\numset{K}^p\)に対し，集合\(\argmin_{\vect{y}\in S}\vnorm{\vect{y}-\vect{x}}\)は\(S\)が部分ベクトル空間のときと同様，ただ1つの元を持つ．
...
よって，集合\(\argmin_{\vect{x}\in S}\vnorm{\vect{x}}\)（\(S=\tilde{\vect{x}}+\ker\mat{A}\)）も元をただ1つ持つ．
...
\(\vect{b}\)を変えると\(\ker\mat{A}\)を平行移動する向きと大きさが変わることを表している．
