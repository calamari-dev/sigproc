...
\begin{proof}
  ...
  \[
    \sum_{m=0}^{N-1}x[m](\lagop^m\delta)[n] = \sum_{m=0}^{N-1}x[n]\kdelta{m}{n}
    = x[n]
  \]
  となる．\(x\)は\(N\)を周期に持つから，この式は\(n\notin\Set{0,\dots,N-1}\)のときも成り立つ．
  よって\(x=\sum_{m=0}^{N-1}x[m]\lagop^m\delta\)なので，\(\basis{D}\)は\(\cycles{\numset{C}}{N}\)の基底である．
\end{proof}
...
\cref{equation:discrete_impulse_response}から\(H=\sum_{m=0}^{N-1}h_m\lagop^m\)なので，基底\(\basis{D}\)に関する\(H\)の表現行列\(\mat{H}\)は
\begin{align*}
  \mat{H} &= \sum_{m=0}^{N-1}h_m\mat{L}^m
  = h_0\imat+h_1\matrice*{\trps{\vect{e}_N} \\ \trps{\vect{e}_1} \\ \trps{\vect{e}_2} \\ \vdots \\ \trps{\vect{e}_{N-1}}}+h_2\matrice*{\trps{\vect{e}_{N-1}} \\ \trps{\vect{e}_N} \\ \trps{\vect{e}_1} \\ \vdots \\ \trps{\vect{e}_{N-2}}}+\dots+h_{N-1}\matrice*{\trps{\vect{e}_2} \\ \trps{\vect{e}_3} \\ \vdots \\ \trps{\vect{e}_N} \\ \trps{\vect{e}_1}} \\
