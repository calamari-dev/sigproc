...
\begin{definition}{フレーム}{frame}\index{ふれーむ@フレーム}
  \(\Phi=\seq{\vect{\phi}_i}_{i\in I}\)を\(H\)上の有限列とする．ある実数\(A,B>0\)が存在し，任意の\(\vect{x}\in H\)に対して
  \[
    A\vnorm{\vect{x}}^2 \leq \sum_{i\in I}\abs{\innerp{\vect{x}}{\vect{\phi}_i}}^2 \leq B\vnorm{\vect{x}}^2\quad(\vnorm{\vect{x}}=\sqrt{\innerp{\vect{x}}{\vect{x}}})
  \]
...
\begin{proposition}{}{frame_characterization}
  \(\seq{\vect{\phi}_i}_{i\in I}\)を\(H\)のフレームとする．このとき，集合\(\Set{\vect{\phi}_i\given i\in I}\)は\(H\)の生成系である．
\end{proposition}

\begin{proof}
  対偶を示す．\(V=\spannedby\Set{\vect{\phi}_i\given i\in I}\)とする．もし\(V\neq H\)なら\(\zvec\)でない\(\vect{x}\in\pcomp{V}\)がある．
  すると，すべての\(i\in I\)で\(\innerp{\vect{x}}{\vect{\phi}_i}=0\)なので，いかなる\(A>0\)についても\(\sum_{i\in I}\abs{\innerp{\vect{x}}{\vect{\phi}_i}}^2=0<A\vnorm{\vect{x}}^2\)となる．
  よってフレーム限界が存在しないので，\(\seq{\vect{\phi}_i}_{i\in I}\)はフレームでない．
\end{proof}

\begin{note}
  実は\cref{proposition:frame_characterization}の逆も成り立つ．つまり，有限次元内積空間の生成系はフレーム限界を持つ（Casazza \cite{casazza2013}に証明がある）．
  証明には，\(\dim H<+\infty\)より\(H\)の単位球面は点列コンパクトであることが効いてくる．
\end{note}
...
\[
  A\vnorm{\vect{x}}^2 = \sum_{i\in I}\abs{\innerp{\vect{x}}{\vect{\phi}_i}}^2\quad(\vect{x}\in H)
\]
となる有限列\(\seq{\vect{\phi}_i}_{i\in I}\)を\termdef{タイトフレーム}\index{たいとふれーむ@タイトフレーム}\index{ふれーむ@フレーム!たいと@タイト\texttwoemdash}（tight frame）という．
\(A=1\)のタイトフレームは\termdef{パーセバルフレーム}\index{ぱーせばるふれーむ@パーセバルフレーム}\index{ふれーむ@フレーム!ぱーせばる@パーセバル\texttwoemdash}（Parseval frame）と呼ばれる．

\begin{example}
  \(\vect{\phi}_k=\trps{\rowvect{\cos(2\krez k/3+\krez/2) & \sin(2\krez k/3+\krez/2)}}\)のとき，列\(\seq{\vect{\phi}_k}_{k=0}^2\)は\(\numset{R}^2\)のタイトフレームである．
...
\begin{definition}{}{frame_analysis_synthesis}\index{ふれーむ@フレーム!さようそ@\texttwoemdash 作用素}\index{ぶんかいさようそ@分解作用素}\index{ごうせいさようそ@合成作用素}
  \(\Phi=\seq{\vect{\phi}_i}_{i\in I}\)を\(H\)のフレームとする．
  \begin{enumerate}
    \item 線形変換\(S\colon H\to H\)，\(S(\vect{x})=\sum_{i\in I}\innerp{\vect{x}}{\vect{\phi}_i}\vect{\phi}_i\)を\(\Phi\)の\termdef{フレーム作用素}（frame operator）という．
    \item 線形写像\(T\colon H\to\mapset{I}{\numset{K}}\)，\(T(\vect{x})=\seq{\innerp{\vect{x}}{\vect{\phi}_i}}\)を\(\Phi\)の\termdef{分解作用素}（analysis operator）という．
    \item 線形写像\(\synth{T}\colon\mapset{I}{\numset{K}}\to H\)，\(\synth{T}(\seq{x_i})=\sum_{i\in I}x_i\vect{\phi}_i\)を\(\Phi\)の\termdef{合成作用素}（synthesis operator）という．
  \end{enumerate}
\end{definition}

\begin{note}
  \cref{definition:frame_analysis_synthesis}は以前に見た，正規直交基底に関する\(T\)と\(\synth{T}\)の拡張になっている．というより，分析（合成）作用素はフレームに対して定義するのが普通である．
\end{note}
