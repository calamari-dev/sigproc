...
\cref{section:dft}から，本書の主題である信号解析に入っていく．

\subsection{信号の空間}

手始めに，いまから扱う信号とは何か明確にしておく．
...
\(I\)と\(O\)を決めたとき，考えられる信号の最も広い枠組みは集合\(\mapset{I}{O}\)であろう．
しかし，\(\mapset{I}{O}\)の元すべてに共通する性質は普通そう多くないし，\(\mapset{I}{O}\)には現実の系でまず見ない，扱いにくい（病的な）写像まで属していることが多い．
そこで我々は，\(\mapset{I}{O}\)の中でも性質がよい元に限って，それらが有する性質を解析する．言い換えると，適当な条件を満たす信号全体\(S\subset\mapset{I}{O}\)が持つ性質を調べる．

\cref{section:dft}で扱うのは，\(I\)が整数か\kenten{その}組からなる信号である．
こうした信号は\termdef{離散時間信号}\index{りさんじかんしんごう@離散時間信号}\index{しんごう@信号!りさんじかん@離散時間\texttwoemdash}（discrete‐time signal）と呼ばれる．
反対に，\(I\)が区間や実数全体のように連続的である信号は\termdef{連続時間信号}\index{れんぞくじかんしんごう@連続時間信号}\index{しんごう@信号!れんぞくじかん@連続時間\texttwoemdash}（continuous‐time signal）と呼ばれる．

時間\(N\)が有限な離散時間信号の全体\(\mapset{\Set{0,\dots,N-1}}{\numset{K}}\)は，数ベクトル空間\(\numset{K}^N\)と同一視できる．これは
\[
  x = \seq{x_n}_{n=0}^{N-1}\longleftrightarrow
  \vect{x} = \trps{\matrice{x_0 & \cdots & x_{N-1}}}
\]
という対応づけを考えれば明らかであろう．また，数列全体\(\mapset{\numset{Z}}{\numset{K}}\)\texttwoemdash これを\termdef{数列空間}\index{すうれつくうかん@数列空間}（sequence space）という\texttwoemdash も，次の演算でベクトル空間になる．
\[
...
\[
  \delta[n] = \sum_{m\in\numset{Z}}\kdelta{n}{mN}
  = \begin{cases}1 & \text{(\(n\)は\(N\)で割り切れる)},\\ 0 & \text{(otherwise)}\end{cases}
\]
...
\[
  ...
  = x[n]
\]
が成立する．よって，集合\(\basis{D}=\Set{\lagop^0\delta,\lagop^1\delta,\dots,\lagop^{N-1}\delta}\)は\(\cycles{\numset{C}}{N}\)の基底である．
