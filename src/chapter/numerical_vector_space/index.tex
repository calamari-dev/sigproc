...
変数\(X\)と変数\(Y\)の間には，多項式で表される関係\(Y=c_0+c_1X^1+\dots+c_pX^p\)があると仮定する．
ただし次数\(p\)は既知であり，各\(X^k\)の係数\(c_k\)を\(X\)と\(Y\)の観測値から推定したい．

たとえば，おもりを自由落下させると落下時間\(T\)と移動距離\(Y\)の間には\(Y=(g/2)T^2\)という関係がある（\(g\)は重力加速度と呼ばれる定数である）．
そこで，\((T,Y)\)の値をくり返し測定し，得られた2次元データを関数\(t\mapsto at^2\)にあてはめれば，\(g=2a\)の値を間接的に測定できる．

次の定理から，\(c_0,\dots,c_p\)を決定するには最低限\(p+1\)個のデータがあればよい．

\begin{theorem}{}{}\index{ほかんたこうしき@補間多項式}
  点列\(\trps{\rowvect{x_0 & y_0}},\dots,\trps{\rowvect{x_p & y_p}}\in\numset{K}^2\)は，相異なる任意の\(i,j\in\Set{0,\dots,p}\)に対して\(x_i\neq x_j\)を満たすとする．
  このとき，すべての\(i\)で\(y_i=P(x_i)\)を満たす，次数が\(p\)以下の\(\numset{K}\)係数多項式\(P(x)\)がただ一つ存在する\footnotemark ．
\end{theorem}

\begin{proof}
  まず\(P(x)\)が存在することを示す．
  \[
    L_k(x) = \frac{x-x_0}{x_k-x_0}\frac{x-x_1}{x_k-x_1}\dotsm\frac{x-x_{k-1}}{x_k-x_{k-1}}\frac{x-x_{k+1}}{x_k-x_{k+1}}\dotsm\frac{x-x_p}{x_k-x_p}
  \]
  とおくと，\(L_k(x_i)=\kdelta{i}{k}\)だから\(P(x)=\sum_{k=0}^py_kL_k(x)\)は条件を満たす．

  次に\(P(x)\)が一意であることを示す．多項式\(P_1(x)\)と\(P_2(x)\)はどちらも条件を満たすとする．
  このとき，多項式\(D(x)=P_1(x)-P_2(x)\)は\(D(x_i)=y_i-y_i=0\)を満たすので，\(D(x)=a(x-x_0)\dotsm(x-x_p)\)（\(a\in\numset{K}\)）と因数分解できる．
  \(D(x)\)の次数は最大でも\(p\)だから\(a=0\)，すなわち\(P_1(x)=P_2(x)\)である．
\end{proof}

\footnotetext{この多項式を\termdef{補間多項式}（interpolating polynomial）という．}
しかし，実験で得られるデータは誤差を含むものだから，普通はデータを\(p+1\)個より多く測定し，平均的にもっともあてはまりがよい\(c_0,\dots,c_p\)の組を求める．
あてはまりのよさを表現する指標はいろいろあるが，ここでは関数
\[
  \epsilon(c_0,\dots,c_p) = \frac{1}{n}\sum_{i=1}^n\abs*{y_i-\sum_{k=0}^pc_kx_i^k}^2
\]
を採用する．
