...
本節では，あるベクトルを他のベクトルの線形結合で近似する手法を説明する．\cref{chapter:numerical_vector_space}において\(\numset{K}\)は\(\numset{R}\)か\(\numset{C}\)を意味し，
...
\cref{proposition:finite_projection}まで，\(H\)を任意の（\(\Set{\zvec}\)でない）有限次元内積空間とする．分かりにくければ\(H=\numset{K}^n\)と思ってよい．
...
\begin{proposition}{}{}
  \(H\)の任意の部分空間\(V\)について，写像\(\proj_V\colon H\to V\)は線形写像である．
\end{proposition}
...
\begin{definition}{直交補空間}{numerical_perpendicular_complement}\index{ちょっこうほくうかん@直交補空間}\indexsymbol{\(\pcomp[H]{V}\)}\indexsymbol{\(\pcomp{V}\)}
...
\[
  ...
  = \proj_{V_m}\vect{x}
\]
であり，\(\proj_{V_m}\vect{x}=\mat{U}\mat{\Delta}\htrps{\mat{U}}\vect{x}\)が成立する．よって，\(\proj_{V_m}\)は線形写像\(T(\vect{x})=\htrps{\mat{U}}\vect{x}\)，\(D(\vect{x})=\mat{\Delta}\vect{x}\)，\(\synth{T}(\vect{x})=\mat{U}\vect{x}\)を用いて\(\proj_{V_m}=\synth{T}DT\)と表せる．
...
明らかに\(\cycles{N}\)は数列空間の部分空間である．\(\cycles{N}\)上の\termdef{ラグ作用素}\index{らぐさようそ@ラグ作用素}（lag operator）\(\lagop\)を\(\lagop x=\seq{x[n-1]}\)で定義し，数列\(\comb\in\cycles{N}\)を
\[
...
\begin{proof}
  ...
  一方，\(H\)の表現行列は\(\mat{H}\)だから\(\dft{N}H\dft*{N}\)の表現行列は\(\htrps{\mat{W}}\mat{H}\mat{W}\)でもある．
...
\cref{section:stft,section:discrete_mra}では，時刻と周波数にまたがって現れる信号の特徴を調べる，\termdef{時間周波数解析}\index{じかんしゅうはすうかいせき@時間周波数解析}（time–frequency analysis）の理論を説明する．
この理論は旋律・地震動・景気など，時が経つにつれ波の様子が移り変わる信号を解析する分野で活用されている．

\subsection{フレーム}
\label{subsection:frame}

\cref{subsection:frame}では，基底を一般化する「フレーム」という対象を，有限次元の内積空間に限って紹介する（より一般的な定義は\cref{xr-chapter:hilbert_space}に回す）．

特に断らない限り，\cref{subsection:frame}では\(H\)を任意の有限次元内積空間とする．

\begin{definition}{フレーム}{frame}\index{ふれーむ@フレーム}
  \(\seq{\vect{e}_i}_{i\in I}\)を\(H\)上の有限列とする．ある実数\(A,B>0\)が存在し，任意の\(\vect{v}\in H\)に対して
  \[
    A\vnorm{\vect{v}}^2 \leq \sum_{i\in I}\abs{\innerp{\vect{v}}{\vect{e}_i}}^2 \leq B\vnorm{\vect{v}}^2\quad(\vnorm{\vect{v}}=\sqrt{\innerp{\vect{v}}{\vect{v}}})
  \]
  を満たすとき，\(\seq{\vect{e}_i}_{i\in I}\)を\(H\)の\termdef{フレーム}（frame）という．
\end{definition}

\(A\)と\(B\)を\(\seq{\vect{e}_i}_{i\in I}\)の\termdef{フレーム限界}\index{ふれーむ@フレーム!げんかい@\texttwoemdash 限界}（frame bound）という．
特に，\(A\)の最大値と\(B\)の最小値を\(\seq{\vect{e}_i}_{i\in I}\)の\termdef{最良フレーム限界}（optimal frame bound）という．

\begin{proposition}{}{frame_characterization}
  \(\seq{\vect{e}_i}_{i\in I}\)を\(H\)のフレームとする．このとき，集合\(\Set{\vect{e}_i\given i\in I}\)は\(H\)の生成系である．
\end{proposition}

\begin{proof}
  対偶を示す．\(V=\spannedby\Set{\vect{e}_i\given i\in I}\)とする．もし\(V\neq H\)なら\(\zvec\)でない\(\vect{v}\in\pcomp{V}\)がある．
  すると，すべての\(i\in I\)で\(\innerp{\vect{v}}{\vect{e}_i}=0\)なので，いかなる\(A>0\)についても\(\sum_{i\in I}\abs{\innerp{\vect{v}}{\vect{e}_i}}^2=0<A\vnorm{\vect{v}}^2\)となる．
  よってフレーム限界が存在しないので，\(\seq{\vect{e}_i}_{i\in I}\)はフレームでない．
\end{proof}

\begin{note}
  実は\cref{proposition:frame_characterization}の逆も成り立つ．つまり，有限次元内積空間の生成系はフレーム限界を持つ（\cite{casazza2013}に証明がある）．
  証明には，\(\dim H<+\infty\)より\(H\)の単位球面はコンパクトであり，単位球面上の点列は収束部分列を持つことが効いてくる．
\end{note}
...
\begin{example}[\(\cycles{N}\)上のDFT]
  ...
  \]
  で定める．すると，数列\(w_k[n]=N^{-1/2}\napr^{2\krez\iuni kn/N}\)がなす集合\(\Set{w_k\given 0\leq k<N}\)は\(\cycles{N}\)の正規直交基底になる（証明は\cref{proposition:dft_onb}の焼き直しになるので省く）．
  このことから，列\(\seq{w_k}_{k=0}^{N-1}\)は\(\cycles{N}\)のフレームといえる．実際
  \[
    v = \sum_{k=0}^{N-1}\innerp{v}{w_k}w_k,
    \quad\vnorm{v}^2 = \sum_{k=0}^{N-1}\abs{\innerp{v}{w_k}}^2\quad(v\in\cycles{N})
  \]
...
\[
  A\vnorm{\vect{v}}^2 = \sum_{i\in I}\abs{\innerp{\vect{v}}{\vect{e}_i}}^2\quad(\vect{v}\in H)
\]
となる有限列\(\seq{\vect{e}_i}_{i\in I}\)を\termdef{タイトフレーム}\index{たいとふれーむ@タイトフレーム}\index{ふれーむ@フレーム!たいと@タイト\texttwoemdash}（tight frame）という．
\(A=1\)のタイトフレームは\termdef{パーセバルフレーム}\index{ぱーせばるふれーむ@パーセバルフレーム}\index{ふれーむ@フレーム!ぱーせばる@パーセバル\texttwoemdash}（Parseval frame）と呼ばれる．

\begin{example}
  \(\vect{e}_k=\trps{\rowvect{\cos(2\krez k/3+\krez/2) & \sin(2\krez k/3+\krez/2)}}\)のとき，列\(\seq{\vect{e}_k}_{k=0}^2\)は\(\numset{R}^2\)のタイトフレームである．
  このフレームは\termdef{メルセデス・ベンツフレーム}\index{めるせですべんつふれーむ@メルセデス・ベンツフレーム}（Mercedes‐Benz frame）と呼ばれている\cite{casazza2013}．
  そのわけは\cref{figure:mercedes_benz}から明らかであろう．
\end{example}
...
\begin{figure}[htbp]
  ...
  \label{figure:mercedes_benz}
\end{figure}

メルセデス・ベンツフレームがタイトであることを示す．\(\vect{e}_0=\trps{\rowvect{0 & 1}}\)，\(\vect{e}_1=\trps{\rowvect{-\sqrt{3} & -1}}/2\)，\(\vect{e}_2=\trps{\rowvect{\sqrt{3} & -1}}/2\)より，各\(\vect{v}=\trps{\rowvect{x & y}}\)に対して
\[
  \sum_{k=0}^2\abs{\innerp{\vect{v}}{\vect{e}_k}}^2 = y^2+\pqty*{-\frac{\sqrt{3}x}{2}-\frac{y}{2}}^2+\pqty*{\frac{\sqrt{3}x}{2}-\frac{y}{2}}^2
\]
である．右辺を整理すると\((3/2)(x^2+y^2)=(3/2)\vnorm{\vect{v}}^2\)となるので，メルセデス・ベンツフレームはフレーム限界\(3/2\)のタイトフレームである．

フレームがあると3つの線形写像が定まる．これは\cref{definition:analysis_and_synthesis}で見た，正規直交基底に対する\(T\)と\(\synth{T}\)の拡張である．

\begin{definition}{}{frame_analysis_synthesis}\index{ふれーむ@フレーム!さようそ@\texttwoemdash 作用素}\index{ぶんかいさようそ@分解作用素}\index{ごうせいさようそ@合成作用素}
  \(\Phi=\seq{\vect{e}_i}_{i\in I}\)を\(H\)のフレームとする．
  \begin{enumerate}
    \item 線形変換\(S\colon H\to H\)，\(S(\vect{x})=\sum_{i\in I}\innerp{\vect{x}}{\vect{e}_i}\vect{e}_i\)を\(\Phi\)の\termdef{フレーム作用素}（frame operator）という．
    \item 線形写像\(T\colon H\to\mapset{I}{\numset{K}}\)，\(T(\vect{x})=\seq{\innerp{\vect{x}}{\vect{e}_i}}\)を\(\Phi\)の\termdef{分解作用素}（analysis operator）という．
    \item 線形写像\(\synth{T}\colon\mapset{I}{\numset{K}}\to H\)，\(\synth{T}(\seq{x_i})=\sum_{i\in I}x_i\vect{e}_i\)を\(\Phi\)の\termdef{合成作用素}（synthesis operator）という．
  \end{enumerate}
\end{definition}
...
2つの整数を引数にとる複素数列（2重数列）全体を\(\mapset{\numset{Z}\times\numset{Z}}{\numset{C}}\)\indexsymbol{\(\mapset{\numset{Z}\times\numset{Z}}{\numset{C}}\)}と表す．
\(\mapset{\numset{Z}\times\numset{Z}}{\numset{C}}\)の元\(x=\seq{x[m,n]}\)で，\(m\)について周期\(M\)，\(n\)について周期\(N\)であるもの全体を\(\doublecycles{M}{N}\)\index{l2MN@\(\doublecycles{M}{N}\)}と書く．
\(m\)の値を固定してできる数列\(\seq{x[m,n]}_{n\in\numset{Z}}\in\mapset{\numset{Z}}{\numset{C}}\)を\(x[m,\holder]\)と表し，\(n\)についても同様に\(x[\holder,n]\)を定めれば，\(\doublecycles{M}{N}\)は
\[
...
\begin{definition}{短時間フーリエ変換}{discrete_stft}\index{たんじかんふーりえへんかん@短時間フーリエ変換}\index{STFT|see{短時間フーリエ変換}}\index{Vw@\(\stft{w}\)}
  ...
  \]
  に写す写像\(\stft[L]{w}\colon\cycles{L}\to\mapset{\numset{Z}\times\numset{Z}}{\numset{C}}\)を\termdef{短時間フーリエ変換}（short‐time Fourier transform; STFT）という．
\end{definition}

\footnotetext{\(\Set{n\in\numset{Z}\given w[n]=0}\neq\numset{Z}\)ということ．\(w[n]=0\)となる\(n\)が存在してもよい．}
...
\begin{corollary}{}{stft_cyclic}
  \(\stft{w}\)は\(\img\stft{w}\subseteq\doublecycles{L}{L}\)を満たす線形写像である．
\end{corollary}

\begin{proof}
  線形性は\cref{proposition:stft_and_shift}からただちにしたがう．\(x\in\cycles{L}\)と\(k,n\in\numset{Z}\)を任意にとる．
...
\(\img\stft{w}\subseteq\doublecycles{L}{L}\)なので，\alert{以降は\(\stft{w}\)の終域を\(\doublecycles{L}{L}\)に変える}．
...
\cref{figure:phone}は「\ltjruby{嘘}{うそ}でしょ」という話声\(x\)の波形，\cref{figure:phone_spectrogram}はある（実数値）窓関数\(w\)に関する\(\abs{(\stft{w}x)[k,n]}^2\)のグラフである\footnote{カノン\cite{canon}によるITAコーパス読み上げ音声を使用した．}．
...
このうち最も下にある横線は，基本周波数（\cref{subsection:dft}参照）が時間変化してできた軌跡である．したがって，この軌跡をなぞれば旋律の時間変化を読み取れる．
...
\[
  \tilde{w}[n] = \begin{cases}w(n/N) & (0\leq n\leq N), \\ 0 & (N\leq n<L)\end{cases}
\]
...
\begin{definition}{離散ガボール変換}{discrete_gabor}\index{りさんがぼーるへんかん@離散ガボール変換}\index{DGT|see{離散ガボール変換}}\index{Gw@\(\dgt{w}\)}
  \((K,N,L)\)を自然数の3つ組，\(w\)を\(\cycles{L}\)の元とする．各\(x\in\cycles{L}\)を
  \begin{align*}
...
\begin{definition}{\(\varepsilon\)‐集中}{}\index{しゅうちゅう@\(\varepsilon\)‐集中}
  ...
  \]
  を満たすとき，\(x\)は集合\(A\subseteq I\)に\termdef{\(\varepsilon\)‐集中}（\(\varepsilon\)‐concentrated）であるという．
\end{definition}

\begin{theorem}{ドノホー・スタークの不確定性原理}{discrete_donoho_stark}\index{どのほーすたーくのふかくていせいげんり@ドノホー・スタークの不確定性原理}\index{ふかくていせいげんり@不確定性原理!どのほーすたーく@ドノホー・スターク}
  ...
  \]
  である．これを\termdef{ドノホー・スタークの不確定性原理}（Donoho‐Stark uncertainity principle）という．
\end{theorem}
...
\begin{enumerate}
  ...
  \end{equation}
  であることを示せ．信号処理ではしばしば\cref{equation:dft_parseval}を\termdef{パーセバルの定理}\index{ぱーせばるのていり@パーセバルの定理}（Parseval's theorem），あるいは\termdef{プランシュレルの定理}\index{ぷらんしゅれるのていり@プランシュレルの定理}（Plancherel's theorem）と呼ぶ．  
\end{enumerate}
