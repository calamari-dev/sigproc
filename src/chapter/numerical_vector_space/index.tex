...
\cref{proposition:dft_shift_theorem}を使うと，数列の積\(x\cdot y=\seq{x_ny_n}_{n\in\numset{Z}}\)と離散フーリエ変換の間にある関係を示せる．形式ばらずに言えば，積は\(\dft{N}\)で次の2項演算に写される．
...
\begin{proof}
  2つめのみ示す．\cref{proposition:dft_convolution_theorem}より\(\dft{N}((x\circconv y)\circconv z)=(\dft{N}(x\circconv y))\cdot\hat{z}=(\hat{x}\cdot\hat{y})\cdot\hat{z}\)である．
  よって\((x\circconv y)\circconv z=\dft{N}^{-1}(\hat{x}\cdot\hat{y}\cdot\hat{z})\)であり，右辺は\(x\circconv (y\circconv z)\)とも等しい．
\end{proof}
