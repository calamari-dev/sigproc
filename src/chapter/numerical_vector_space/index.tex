...
\begin{proof}
  \(\zeta=\napr^{2\krez\iuni/L}\)，\(\midx{x}{a}{b}=\modulate{aK}\translate{bN}x\)，\(\midx{w}{i}{j}=\modulate{iK}\translate{jN}w\)とおく．すると
  \begin{align*}
    S(\midx{x}{a}{b}) &= \sum_{i,j}\innerp{\midx{x}{a}{b}}{\midx{w}{i}{j}}\midx{w}{i}{j}
    = \sum_{i,j}\sum_n\midx{x}{a}{b}[n]\midx{\conj{w}}{i}{j}[n]\midx{w}{i}{j} \\
    &= \sum_{i,j}\sum_n\zeta^{aKn}x[n-bN]\midx{\conj{w}}{i}{j}[n]\midx{w}{i}{j}
  \end{align*}
  である．\(m=n-bN\)とおき，\(\sum_n\)を\(\sum_m\)に変えると
  \[
    S(\midx{x}{a}{b}) = \sum_{(i,j)\in\Lambda}\sum_mx[m]\conj*{\zeta^{-aK(m+bN)}\midx{w}{i}{j}[m+bN]}\midx{w}{i}{j}
  \]
  となる．さらに\((\translate{t}\modulate{\xi}x)[n]=\zeta^{\xi(n-t)}x[n-t]\)（\(t,\xi\in\numset{Z}\)）なので
  \begin{gather*}
    (\translate{-bN}\modulate{-aK}\midx{w}{i}{j})[m] = \zeta^{-aK(m+bN)}\midx{w}{i}{j}[m+bN], \\
    S(\midx{x}{a}{b}) = \sum_{i,j}\sum_mx[m]\conj*{(\translate{-bN}\modulate{-aK}\midx{w}{i}{j})[m]}\midx{w}{i}{j}, \\
    \translate{-bN}\modulate{-aK}S(\midx{x}{a}{b}) = \sum_{i,j}\innerp{x}{\translate{-bN}\modulate{-aK}\midx{w}{i}{j}}\translate{-bN}\modulate{-aK}\midx{w}{i}{j}
  \end{gather*}
  であり，\((\translate{t}\modulate{\xi}x)[n]=\zeta^{-\xi t}\zeta^{\xi n}x[n-t]=\zeta^{-\xi t}(\modulate{\xi}\translate{t}x)[n]\)より
  \begin{gather*}
    \begin{aligned}
      \translate{-bN}\modulate{-aK}\midx{w}{i}{j} &= \translate{-bN}\modulate{-aK}\modulate{iK}\translate{jN}w
      = (\translate{-bN}\modulate{(i-a)K})\translate{jN}w \\
      &= (\zeta^{(i-a)K(-bN)}\modulate{(i-a)K}\translate{-bN})\translate{jN}w \\
      &= \zeta^{(a-i)KbN}\modulate{(i-a)K}\translate{(j-b)N}w
      = \zeta^{(a-i)KbN}\midx{w}{i-a}{j-b},
    \end{aligned} \\
    \translate{-bN}\modulate{-aK}S(\midx{x}{a}{b}) = \sum_{i,j}\innerp{x}{\midx{w}{i-a}{j-b}}\midx{w}{i-a}{j-b}
  \end{gather*}
  である．この値は\(i-a\)を\(i\)に，\(j-b\)を\(j\)に置き換えても変わらないので\(\translate{-bN}\modulate{-aK}S(\midx{x}{a}{b})=S(x)\)，\(S(\midx{x}{a}{b})=\modulate{aK}\translate{bN}S(x)\)である．
\end{proof}
...
\cref{corollary:dgt_reconstruction}から，\(\gaborsys{w}{K}{N}\)がフレームであれば，\(\dgt{w}x\)から\(x\)は新たなガボールシステム\(\gaborsys{\gamma}{K}{N}\)によって再構成できる．
