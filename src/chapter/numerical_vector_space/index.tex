...
\begin{lemma}{}{grammian_equals_zero}
  \(G\)は\(H\)上の線形変換で，任意の\(\vect{x},\vect{y}\in H\)に対し\(\innerp{\vect{x}}{G(\vect{y})}=\innerp{G(\vect{x})}{\vect{y}}\)かつ\(\innerp{\vect{x}}{G(\vect{x})}=0\)であるとする．
...
\begin{definition}{離散ガボール変換}{discrete_gabor}\index{りさんがぼーるへんかん@離散ガボール変換}\index{DGT|see{離散ガボール変換}}\index{Gw@\(\dgt{w}\)}
  自然数\(K\)と\(N\)は\(L\)を割り切るとする．各\(x\in\cycles{L}\)を
  \begin{align*}
