...
こうした信号を\termdef{離散時間信号}\index{りさんじかんしんごう@離散時間信号}\index{しんごう@信号!りさんじかん@離散時間\texttwoemdash}（discrete‐time signal）という．
反対に，\(I\)が区間\(\ccival{-\krez}{\krez}\)や\(\numset{R}\)のように連続的である信号は\termdef{連続時間信号}\index{れんぞくじかんしんごう@連続時間信号}\index{しんごう@信号!れんぞくじかん@連続時間\texttwoemdash}（continuous‐time signal）という．
...
\]
なので，基底\(\basis{D}\)に関する\(\lagop\)の表現行列\(\mat{L}\)は\(\mat{L}=\trps{\rowvect{\vect{e}_N & \vect{e}_1 & \cdots & \vect{e}_{N-1}}}\)と書ける．
同様に計算すれば，\(0\leq m<N\)のとき\(\lagop^m\)の表現行列は
\[
  \mat{L}^m = \trps{\matrice{\vect{e}_{N-m+1} & \cdots & \vect{e}_N & \vect{e}_1 & \cdots & \vect{e}_{N-m}}}
  = \matrice*{ & \imat_m \\ \imat_{N-m} & }
\]
であることも分かる．
...
\[
  \mat{H} = \sum_{m=0}^{N-1}h_m\matrice*{ & \imat_m \\ \imat_{N-m} & }
...
\begin{proof}
  ...
  \(\cycles{\numset{C}}{N}\)上の線型写像\(H\)を\(H(x)=h\circconv x\)で定義すると，\cref{proposition:dft_convolution_theorem}より
  \[
    (\dft{N}H\dft{N}^{-1})x = \dft{N}(h\circconv\dft{N}^{-1}x)
    = \hat{h}\cdot x\quad(\hat{h}=\dft{N}h)
  \]
  となる．よって，\(\dft{N}H\dft{N}^{-1}\)の表現行列は\(\diag(\hat{h}[0],\dots,\hat{h}[N-1])\)である．
...
\begin{example}
  ...
  平均をとると細かい変動が均されるので，高周波の成分は減るだろう．
  実際\(\abs{\hat{h}[k]}=(1/3)\abs{\zeta^{-k\cdot 0}+\zeta^{-k\cdot 1}+\zeta^{-k\cdot 2}}\)のグラフは\cref{figure:filter_characteristics}のようになり，\(\dft{N}(h\circconv x)=\hat{h}\cdot\hat{x}\)だから\(h\)を畳み込むと高周波は削られる．
\end{example}
...
ただし，\(\mapset{\Omega}{\numset{C}}\)の内積は1変数のときと同様\(\innerp{x}{y}=\sum_{\vect{r}\in\Omega}x[\vect{r}]\conj*{y[\vect{r}]}\)で定義する．
...
\begin{proof}
  \(\vect{i}=\trps{\rowvect{i_1 & \cdots & i_d}}\)，\(\vect{j}=\trps{\rowvect{j_1 & \cdots & j_d}}\)を\(\Omega\)の勝手な2元とする．
  各\(\vect{r}=\trps{\rowvect{r_1 & \cdots & r_d}}\in\Omega\)に対して
  \begin{align*}
    w_{\vect{i}}[\vect{r}]\conj*{w_{\!\vect{j}}[\vect{r}]} &= \frac{\napr^{2\krez\iuni\trps{(\vect{i}-\vect{j})}\mat{N}^{-1}\vect{r}}}{\det\mat{N}}
    = \frac{1}{N_1\dotsm N_d}\exp\pqty*{\sum_{k=1}^d\frac{2\krez\iuni(i_k-j_k)r_k}{N_k}} \\
    &= \prod_{k=1}^d\pqty*{\frac{1}{N_k}\exp\pqty*{\frac{2\krez\iuni(i_k-j_k)r_k}{N_k}}}
  \end{align*}
  だから\indexsymbol{\(\prod\)}\footnote{\(\prod\)は総乗\(\prod_{i=1}^na_i=a_1\times\dots\times a_n\)を意味する．\(\sum\)の乗法バージョン．}，\(\zeta_k=\napr^{2\krez\iuni/N_k}\)とおくと
  \[
  ...
  \]
  となる．よって\cref{proposition:dft_onb}より\(\innerp{w_{\vect{i}}}{w_{\!\vect{j}}}=\prod_{k=1}^d\kdelta{i_k}{j_k}=\kdelta{\vect{i}}{\vect{j}}\)である．
\end{proof}
