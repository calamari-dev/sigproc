...
この方程式の解を1つ選び\(\tilde{\vect{x}}\)とおく．すると，他の解\(\vect{x}\)は\(\mat{A}(\vect{x}-\tilde{\vect{x}})=\tilde{\vect{b}}-\tilde{\vect{b}}=\zvec\)，
つまり\(\vect{x}-\tilde{\vect{x}}\in\nulsp\mat{A}\)（\(\nulsp\mat{A}=\Set{\vect{x}\in\numset{C}^p\given\mat{A}\vect{x}=\zvec}\)）\index{nul@\(\nulsp\mat{A}\)}を満たす．
