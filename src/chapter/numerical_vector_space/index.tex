...
\(\innerp{\holder}{\holder}\)は\(\numset{K}^n\)の標準内積を意味する．また，\(\vnorm{\vect{x}}=\sqrt{\innerp{\vect{x}}{\vect{x}}}\)を\(\vect{x}\)の\termdef{ノルム}\index{ノルム}\indexsymbol{\(\vnorm{\holder}\)}（norm）と呼ぶ．
...
\[
  ...
  = \sum_{i=1}^r(\sigma_iy_i-\htrps{\vect{u}_i}\vect{b})^2+\sum_{i=r+1}^n(\htrps{\vect{u}_i}\vect{b})^2
\]
となる．右辺の値が最も小さくなるのは\(y_i=(1/\sigma_i)\htrps{\vect{u}_i}\vect{b}\)（\(1\leq i\leq r\)）のときであり，\(y_{r+1},\dots,y_n\)の値はなんであってもかまわない．
\(\vect{x}=\mat{V}\vect{y}\)なので，\(\vnorm{\vect{\epsilon}(\vect{x})}\)が最小値をとるのは\(\vect{x}\)が
\begin{align*}
  \vect{x} &= \mat{V}\matrice{(1/\sigma_1)\htrps{\vect{u}_1}\vect{b} & \cdots & (1/\sigma_r)\htrps{\vect{u}_r}\vect{b} & y_{r+1} & \cdots & y_n} \\
  &= \mat{V}\matrice*{\diag(1/\sigma_1,\dots,1/\sigma_r,a_{r+1},\dots,a_k) & \\ & \zmat}\htrps{\mat{U}}\vect{b}
\end{align*}
の形で表せるときである．ただし，\(a_{r+1},\dots,a_k\)（\(k=\min\Set{n,p}\)）は任意の複素数である．

以上の議論をまとめると，次のことが分かる．

\begin{proposition}{}{}
  \(k=\min\Set{n,p}\)とする．\(\mat{A}\neq\zmat\)を仮定し，\(\mat{A}\)の特異値分解を\(\mat{A}=\mat{U}\smallmatrice{\diag(\sigma_1,\dots,\sigma_r) & \\ & \zmat}\htrps{\mat{V}}\)（\(\sigma_r>0\)）とおく．
  このとき，\(\argmin_{\vect{x}\in\numset{K}^p}\vnorm{\vect{\epsilon}(\vect{x})}\)は次のように表せる．
  \[
    \Set*{\mat{V}\matrice*{\diag(1/\sigma_1,\dots,1/\sigma_r,a_{r+1},\dots,a_k) & \\ & \zmat}\htrps{\mat{U}}\vect{b}\given a_{r+1},\dots,a_k\in\numset{C}}
  \]
\end{proposition}
