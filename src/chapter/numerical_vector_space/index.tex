...
\begin{proof}
  ...
  \begin{align*}
    \vnorm{\vect{\epsilon}(\vect{z})}^2 &= \vnorm*{\sum z_i\vect{e}_i-\vect{x}}^2
    = \vnorm*{\sum z_i\vect{e}_i}^2-2\rpart\innerp*{\sum z_i\vect{e}_i}{\vect{x}}+\vnorm{\vect{x}}^2 \\
    &= \sum\abs{z_i}^2-2\sum\rpart[z_i\innerp{\vect{e}_i}{\vect{x}}]+\vnorm{\vect{x}}^2
  \end{align*}
  である．よって，\(\vnorm{\vect{\epsilon}(\vect{z})}^2\)は\(s_i=\rpart z_i\)と\(t_i=\ipart z_i\)の式で
  \begin{align*}
    \vnorm{\vect{\epsilon}(\vect{z})}^2 &= \vnorm{\vect{x}}^2+\sum(-2\rpart[(s_i+\iuni t_i)\conj*{\innerp{\vect{x}}{\vect{e}_i}}]+s_k^2+t_k^2) \\
