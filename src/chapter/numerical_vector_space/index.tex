...
\subsection{直交補空間}
\begin{definition}{直交補空間}{numerical_perpendicular_complement}\index{ちょっこうほくうかん@直交補空間!かずべくとるくうかんじょうの@数ベクトル空間上の—}\index{\(\pcomp{W}\)}\index{\(\pcomp[V]{W}\)}
...
\subsection{スペクトル定理}
直交射影\(\proj_V(\vect{x})\)を具体的な式で表そう．そのためにまず，定義をいくつか導入する．
...
\begin{proof}
  \(\htrps{\mat{U}}\mat{U}=[\midx{v}{i}{j}]\)とおく．このとき
  \[
  ...
  \]
  なので，\(\midx{v}{i}{j}=\htrps{\vect{u}_i}\vect{u}_j=\innerp{\vect{u}_j}{\vect{u}_i}\)である．
  よって，\(\mat{U}^{-1}=\htrps{\mat{U}}\)であることと，各\(i,j\in\Set{1,\dots,n}\)に対して\(\innerp{\vect{u}_i}{\vect{u}_j}=\kdelta{i}{j}\)であることは同値である．
\end{proof}
