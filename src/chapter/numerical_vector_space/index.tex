...
\begin{proof}
  ...
  \end{align*}
  なので\(e(\innerp{\vect{x}-\vect{n}}{\vect{v}})=-\abs{\innerp{\vect{x}-\vect{n}}{\vect{v}}}^2\geq 0\)，よって\(\innerp{\vect{x}-\vect{n}}{\vect{v}}=0\)である．
\end{proof}
...
\begin{example}
  \(\vect{v}_1=\trps{\inlinevec{1 & 1 & 1}}\)，\(\vect{v}_2=\trps{\inlinevec{2 & -1+\iuni\sqrt{3} & -1-\iuni\sqrt{3}}}\)とし，
...
\subsection{直交補空間}
\begin{definition}{直交補空間}{numerical_perpendicular_complement}\index{ちょっこうほくうかん@直交補空間!かずべくとるくうかんじょうの@数ベクトル空間}\index{\(\pcomp{W}\)}\index{\(\pcomp[V]{W}\)}
...
一方で（特に信号解析では），\(\vect{x}\)の性質を調べるのに利用したい正規直交系\(\basis{B}\)があって，そこから\(V=\spannedby\basis{B}\)への直交射影\(\proj_V(\vect{x})\)を作ることも多い．そのような場合，直交射影は3つの操作に分解できる．
...
集合\(\basis{B}=\Set{\vect{e}_1,\dots,\vect{e}_m}\)を\(\numset{C}^n\)の（基底とは限らない）正規直交系とし，\(V=\spannedby\basis{B}\)とおく．
このとき
\[
  \proj_V(\vect{x}) = \sum_{i=1}^m\vect{e}_i\innerp{\vect{x}}{\vect{e}_i}
  = \sum_{i=1}^m\vect{e}_i\conj{\innerp{\vect{e}_i}{\vect{x}}}
  = \sum_{i=1}^m\vect{e}_i\htrps{\vect{e}_i}\vect{x}
  = \matrice{\vect{e}_1 & \cdots & \vect{e}_m}\matrice*{\htrps{\vect{e}_1} \\ \vdots \\ \htrps{\vect{e}_m}}\vect{x}
\]
である．ここで，\(\basis{B}\)を延長して，\(\numset{C}^n\)の正規直交基底\(\basis{B}'=\Set{\vect{e}_1,\dots,\vect{e}_n}\)を作る．
\(m\times n\)行列\(\mat{A}=\inlinevec{\vect{a}_1 & \cdots & \vect{a}_n}\)を\(\mat{A}=\inlinevec{\imat & \zmat}\)で定義すると
\[
  \matrice{\vect{e}_1 & \cdots & \vect{e}_n}\htrps{\mat{A}} = \matrice{\vect{e}_1 & \cdots & \vect{e}_n}\matrice*{\htrps{\vect{a}_1} \\ \vdots \\ \htrps{\vect{a}_n}}
  = \sum_{i=1}^n\vect{e}_i\htrps{\vect{a}_i}
  = \matrice{\vect{e}_1 & \cdots & \vect{e}_m}
\]
である．よって，\(\mat{U}=\htrps{\inlinevec{\vect{e}_1 & \cdots & \vect{e}_n}}\)，\(\mat{\Lambda}=\htrps{\mat{A}}\mat{A}=\diag(1,\dots,1,0,\dots,0)\)とおくと\(\proj_V(\vect{x})=\htrps{\mat{U}}\mat{\Lambda}\mat{U}\vect{x}\)が成立する．
言い換えれば，\(\proj_V\)は\(\numset{C}^n\)から\(\numset{C}^n\)への3つの写像
\[
  U(\vect{x}) = \mat{U}\vect{x} = \matrice*{\innerp{\vect{x}}{\vect{e}_1} \\ \vdots \\ \innerp{\vect{x}}{\vect{e}_n}},
  \quad L(\vect{x}) = \mat{\Lambda}\vect{x} = \matrice*{x_1 \\ \vdots \\ x_m \\ \zvec}, \\
  \quad\synth{U}(\vect{x}) = \htrps{\mat{U}}\vect{x} = \sum_{i=1}^nx_i\vect{e}_i
\]
を用いて，\(\proj_V=\synth{U}LU\)と表せる．
...
\begin{definition}{正規行列，ユニタリ行列}{regular_and_unitary_matrix}\index{せいきぎょうれつ@正規行列}\index{ゆにたりぎょうれつ@ユニタリ行列}
  ...
  \end{enumerate}
\end{definition}
\footnotetext{エルミート転置は\termdef{随伴行列}（adjoint matrix）と呼ばれることも多いが，別の行列を随伴行列と呼ぶ流儀もあり，まぎらわしい．そのため，本書はエルミート転置で統一する．}

\cref{definition:regular_and_unitary_matrix}から，ユニタリ行列は正規行列である．また，次の命題が成立する．

\begin{proposition}{ユニタリ行列の性質}{}
  \(\mat{U}=\inlinevec{\vect{u}_1 & \cdots & \vect{u}_n}\)を\(n\)次複素正方行列とする．このとき，以下の命題は同値である．
  \begin{enumerate}
