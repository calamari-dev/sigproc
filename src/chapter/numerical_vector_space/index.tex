...
\(S^{-1}(w)\)を\(w\)の\termdef{標準双対窓}\index{ひょうじゅんそうついまど@標準双対窓}（canonical dual window）という．
...
実は，この傾向はハン窓に限ったものではなく，短時間フーリエ変換において，時間方向の明瞭さと，周波数方向の明瞭さは両立できない．この事実を\termdef{不確定性原理}\index{ふかくていせいげんり@不確定性原理}（uncertainty principle）という．

\begin{note}
  不確定性原理は，不確定性の定義に応じて複数の定式化がある．その一つが，\cref{xr-chapter:hilbert_space}で紹介するハイゼンベルクの不確定性原理\index{ふかくていせいげんり@不確定性原理!はいぜんべるく@ハイゼンベルク}である．
\end{note}
