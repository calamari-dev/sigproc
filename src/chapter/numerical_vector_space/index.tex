...
\begin{definition}{直交補空間}{numerical_perpendicular_complement}\index{ちょっこうほくうかん@直交補空間}\index{\(\pcomp{V}\)}\index{\(\pcomp[V]{W}\)}
...
\begin{definition}{離散フーリエ変換}{discrete_fourier_transform}\index{りさんふーりえへんかん@離散フーリエ変換}\index{でぃーえふてぃー@DFT|see{離散フーリエ変換}}\index{\(\dft{n}\vect{x}\)}
  \[
