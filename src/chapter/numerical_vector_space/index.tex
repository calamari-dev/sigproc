...
ここまで説明した\(\argmin_{\vect{x}\in\numset{K}^p}\vnorm{\mat{A}\vect{x}-\vect{b}}\)に関する理論は，データを曲線にあてはめるとき威力を発揮する．
...
\(\vect{x}_i=\trps{\rowvect{1 & x_i & \cdots x_i^p}}\)，\(\mat{X}=\trps{\rowvect{\vect{x}_1 & \cdots & \vect{x}_n}}\)とおくと，\(f(\vect{c};x_i)=\trps{\vect{x}_i}\vect{c}\)だから
\[
...
\]
である．よって，\(f(\vect{c};x)\)が標本に最もよくあてはまる\(\vect{c}\)を求めるには，さきほど扱った方法で\(\argmin_{\vect{c}\in\numset{K}^{p+1}}\vnorm{\mat{X}\vect{c}-\vect{y}}\)を計算すればよい．

同じ考え方は，\(y\)が\(x\)の多項式でなくても使える．変数\(y\)は，変数\(x\)を既知の関数\(\phi_1,\dots,\phi_p\)で写像した線型結合によって
\begin{equation}
  \label{equation:linear_combination_of_functions}
  y = c_1\phi_1(x)+\dots+c_p\phi_p(x)\quad(c_1,\dots,c_p\in\numset{K})
\end{equation}
と書けるとする．関数\(\epsilon(\vect{c})=\sum_{i=1}^n\abs{f(\vect{c};x_i)-y_i}\)（\(f(\vect{c};x)=\sum_{k=1}^pc_k\phi_k(x)\)）の値を最小にする\(\vect{c}\in\numset{K}^p\)全体は，
集合\(\argmin_{\vect{c}\in\numset{K}^p}\vnorm{\mat{X}\vect{c}-\vect{y}}\)である．ただし，\(\mat{X}\)は\(n\times p\)行列
\[
  \mat{X} = \matfence{\phi_k(x_i)}
