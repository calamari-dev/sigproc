...
\(\spannedby S\)と表記する．この言葉を使えば，先述した\(\basis{B}_n\)が持つ性質を「\(\spannedby\basis{B}_n=\numset{R}^n\)が成り立つ」とも言い換えられる．
...
\(\numset{R}^2\)である．しかし，\(\basis{B}_2\)の元の線型結合で\(\numset{R}^2\)の元を表す方法はただ1通りであるのに対して，\(S\)はこの性質を持たない（\cref{figure:linear_comb}）．
...
\begin{figure}[htbp]
  ...
  \end{minipage}
  \caption{\(S\)の元の線型結合で\(\vect{x}=\trps{[\begin{matrix}3/2 & 0\end{matrix}]}\)を表した様子．明らかに\(\vect{x}=(-3/2)\vect{v}_3\)である一方，\(\vect{x}=(\vect{v}_1+\vect{v}_2)/2\)も成り立つ．}
