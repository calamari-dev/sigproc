...
\begin{proof}
  \(\innerp{\vect{w}_i}{\vect{w}_j}=\kdelta{i}{j}\)を示すのはめんどうなので，\(\innerp{\vect{w}_{j-1}}{\vect{w}_j}=0\)のみ示す．
  ...
  \[
    \innerp{\vect{w}_{j-1}}{\vect{w}_j} = \sum_{k=0}^{n-1}\frac{\zeta^{-(j-1)k}}{\sqrt{n}}\frac{\conj{\zeta^{-jk}}}{\sqrt{n}}
...
\begin{definition}{多次元離散フーリエ変換}{multidimensional_discrete_fourier_transform}\index{りさんふーりえへんかん@離散フーリエ変換!たじげん@多次元}
  自然数\(n_1,\dots,n_d\)に対して\(\mat{N}=\diag(n_1,\dots,n_d)\)，\(I=\Set{\trps{\rowvect{x_1 & \cdots & x_d}}\in\numset{Z}^d\given 0\leq x_j<n_j}\)とおく．
  関数\(f\colon I\to\numset{C}\)に対して，関数
  \[
    \hat{f}(\vect{\nu}) = \frac{1}{\sqrt{\det\mat{N}}}\sum_{\vect{x}\in I}f(\vect{x})\napr^{-2\krez\iuni j\innerp{\mat{N}^{-1}\vect{\nu}}{\vect{x}}}\quad(\vect{\nu}\in I)
  \]
  を対応づける線型写像を\termdef{\(d\)次元離散フーリエ変換}という．
\end{definition}
