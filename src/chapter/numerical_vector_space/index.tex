...
\begin{note}
  ...
  \]
  である．よって，正規方程式は\(\gradof\epsilon(\vect{x})=\sum_{i=1}^p(\difsp{\epsilon}{x_i})(\vect{x})\vect{e}_i=\zvec\)と等価である．
\end{note}
...
\begin{corollary}{特異値分解}{singular_value_decomposition}\index{とくいちぶんかい@特異値分解}
...
\begin{proof}
  ...
  \(\mat{V}=\rowvect{\vect{v}_1 & \cdots & \vect{v}_p}\)，\(\mat{\Lambda}=\diag(\lambda_1,\dots,\lambda_p)\)とおく．\(\lambda_i\vect{v}_i=(\htrps{\mat{A}}\mat{A})\vect{v}_i\)，\(\kdelta{i}{j}=\trps{(\trps{\vect{v}_i}\conj{\vect{v}}_j)}=\htrps{\vect{v}_j}\vect{v}_i\)より
  \begin{equation}
    \label{equation:image_orthogonality}
    \lambda_i\kdelta{i}{j} = \htrps{\vect{v}_j}(\htrps{\mat{A}}\mat{A})\vect{v}_i
    = \trps{(\trps{(\mat{A}\vect{v}_i)}\conj*{\mat{A}\vect{v}_j})}
...
特異値分解に基づいて\(\argmin_{\vect{x}\in\numset{K}^p}\vnorm{\vect{\epsilon}(\vect{x})}\)を求めよう．
\(\mat{A}\neq\zmat\)を仮定し，\(\mat{A}\)の特異値分解を\(\mat{A}=\mat{U}\mat{\Sigma}\htrps{\mat{V}}\)とおく．
\cref{proposition:unitary_matrix_property}から，ユニタリ行列を乗じてもノルムは変わらないので
...
よって，\(\vect{x}=\mat{V}\vect{y}\)，\(\mat{U}=\rowvect{\vect{u}_1 & \cdots & \vect{u}_n}\)，\(\mat{\Sigma}=\smallmatrice{\diag(\sigma_1,\dots,\sigma_r) & \\ & \zmat}\)（\(\sigma_r>0\)）とすれば
\[
  \vnorm{\vect{\epsilon}(\vect{x})}^2 = \vnorm{\mat{\Sigma}\vect{y}-\htrps{\mat{U}}\vect{b}}^2
  = \sum_{i=1}^r(\sigma_iy_i-\htrps{\vect{u}_i}\vect{b})^2+\sum_{i=r+1}^p(\htrps{\vect{u}_i}\vect{b})^2
\]
となる．右辺の値が最も小さくなるのは\(y_i=\htrps{\vect{u}_i}\vect{b}/\sigma_i\)（\(1\leq i\leq r\)）のときであり，\(y_{r+1},\dots,y_p\)の値はなんであっても構わない．
\(\vect{x}=\mat{V}\vect{y}\)なので，\(\vnorm{\vect{\epsilon}(\vect{x})}\)が最小値をとるのは\(\vect{x}\)が
\begin{equation}
  \label{equation:explicit_pseudo_inverse}
...
\end{equation}
と表せるときである．\(\vect{z}=\trps{\rowvect{z_1 & \cdots & z_p}}\)，\(\pinv{\mat{\Sigma}}=\smallmatrice{\diag(1/\sigma_1,\dots,1/\sigma_r) & \\ & \zmat}\)，\(\pinv{\mat{A}}=\mat{V}\pinv{\mat{\Sigma}}\htrps{\mat{U}}\)とおいて\cref{equation:explicit_pseudo_inverse}を整理すると
\begin{align*}
  ...
  = \mat{V}\pinv{\mat{\Sigma}}\htrps{\mat{U}}\vect{b}+(\imat-\mat{V}\pinv{\mat{\Sigma}}\mat{\Sigma}\htrps{\mat{V}})\mat{V}\vect{z} \\
  &= \pinv{\mat{A}}\vect{b}+(\imat-\pinv{\mat{A}}\mat{A})\vect{w}\quad(\vect{w}=\mat{V}\vect{z})
\end{align*}
が得られる．\(\pinv{\mat{A}}\)を\(\mat{A}\)の擬似逆行列という．
...
\begin{definition}{擬似逆行列}{}\index{むーあぺんろーずぎゃくぎょうれつ@ムーア・ペンローズ逆行列}\index{ぎじぎゃくぎょうれつ@擬似逆行列}\indexsymbol{\(\pinv{\mat{A}}\)}
  ...
  \(\mat{A}\neq\zmat\)のときは，\(\mat{\Sigma}=\smallmatrice{\mat{\Lambda} & \\ & \zmat}\)を満たす正則な対角行列\(\mat{\Lambda}\)を用いて\(\pinv{\mat{\Sigma}}=\smallmatrice{\mat{\Lambda}^{-1} & \\ & \zmat}\)とする．
...
普通，特異値分解はもとの行列に対して一意ではない．しかし，擬似逆行列はもとの行列に対して一意に定まる（証明は\cref{subsection:uniqueness_of_the_pseudoinverse}を参照せよ）．
さきほどの議論から分かることは，擬似逆行列によって次のようにまとめられる．

\begin{proposition}{}{}
  任意の\(n\times p\)複素行列\(\mat{A}\)に対して，次の式が成立する．
  \[
    \argmin_{\vect{x}\in\numset{K}^p}\vnorm{\mat{A}\vect{x}-\vect{b}} = \Set{\pinv{\mat{A}}\vect{b}+(\imat-\pinv{\mat{A}}\mat{A})\vect{w}\given\vect{w}\in\numset{C}^p}\quad(\vect{b}\in\numset{C}^n)
  \]
\end{proposition}
...
\cref{proposition:unitary_matrix_property}と\cref{proposition:dft_onb}から，次の系がしたがう．
...
\subsection{擬似逆行列の一意性}
\label{subsection:uniqueness_of_the_pseudoinverse}

\begin{proposition}{擬似逆行列の特徴づけ}{}
  以下の条件すべてを満たす\(p\times n\)行列\(\mat{B}\)は，任意の\(n\times p\)行列\(\mat{A}\)に対し一意である．
  また，\(\mat{A}\)の擬似逆行列は条件を満たす．つまり，以下の条件は\(\mat{B}=\pinv{\mat{A}}\)と同値である．
  \begin{enumerate}
    \item \(\mat{A}\mat{B}\mat{A}=\mat{A}\)
    \item \(\mat{B}\mat{A}\mat{B}=\mat{B}\)
    \item \(\htrps{(\mat{A}\mat{B})}=\mat{A}\mat{B}\)
    \item \(\htrps{(\mat{B}\mat{A})}=\mat{B}\mat{A}\)
  \end{enumerate}
\end{proposition}

\begin{proof}
  \(\mat{A}=\mat{U}\mat{\Sigma}\htrps{\mat{V}}\)を\(\mat{A}\)の特異値分解とすると，\(\mat{B}=\mat{V}\pinv{\mat{\Sigma}}\htrps{\mat{U}}\)が上の4条件を満たすことはすぐ確かめられる．
  \(p\times n\)行列\(\mat{B}_1\)，\(\mat{B}_2\)が条件をすべて満たすとき，\(i\)番目の条件から分かる等号を\(\stackrel{i}{=\joinrel=}\)と書くと
  \begin{align*}
    \mat{B}_1 &\stackrel{2,3}{=\joinrel=} \mat{B}_1\htrps{(\mat{A}\mat{B}_1)}
    \stackrel{1}{=\joinrel=} \mat{B}_1\htrps{\mat{B}_1}\htrps{(\mat{A}\mat{B}_2\mat{A})}
    = \mat{B}_1\htrps{(\mat{A}\mat{B}_1)}\htrps{(\mat{A}\mat{B}_2)}
    \stackrel{2,3}{=\joinrel=} \mat{B}_1\mat{A}\mat{B}_2 \\
    &\stackrel{2,4}{=\joinrel=} \htrps{(\mat{B}_1\mat{A})}(\mat{B}_2\mat{A}\mat{B}_2)
    \stackrel{4}{=\joinrel=} \htrps{\mat{A}}\htrps{\mat{B}_1}\htrps{(\mat{B}_2\mat{A})}\mat{B}_2
    = \htrps{(\mat{A}\mat{B}_1\mat{A})}\htrps{\mat{B}_2}\mat{B}_2 \\
    &\stackrel{1}{=\joinrel=} \htrps{\mat{A}}\htrps{\mat{B}_2}\mat{B}_2
    = \htrps{(\mat{B}_2\mat{A})}\mat{B}_2
    \stackrel{2}{=\joinrel=} \mat{B}_2
  \end{align*}
  なので\(\mat{B}_1=\mat{B}_2\)である．
\end{proof}
