...
\begin{definition}{直交補空間}{numerical_perpendicular_complement}\index{ちょっこうほくうかん@直交補空間}\indexsymbol{\(\pcomp[V]{W}\)}\indexsymbol{\(\pcomp{V}\)}
  ...
  \]
  も\(V\)の部分空間になる．この集合を（\(V\)における）\(W\)の\termdef{直交補空間}（orthogonal complement）といい，\(\pcomp[V]{W}\)，\(\pcomp{W}\)などと書く．
\end{definition}
...
\]
で近似しよう．標本化周波数\(N\)で\(f\)を標本化すると，離散時間信号\(x[n]=f(t_n)\)（\(t_n=2\krez n/N\)）が得られる\footnote{信号処理では慣習的に，時刻を表す添字を大かっこで書く．}．
...
\[
  \epsilon(\vect{c}) = \sum_{n=0}^{N-1}\abs{f(\vect{c};t_n)-x[n]}^2\quad(t_n=2\krez n/N)
\]
...
\begin{note}\index{ふーりえきゅうすうてんかい@フーリエ級数展開}
  標本化周波数\(N\)の値を大きくすれば，より多くの標本，より高周波の複素指数関数を近似に使える．
  では，\(N\to\infty\)の極限をとると何が起こるだろう\？ エイリアシングを考慮して極限をとると
  \begin{gather*}
