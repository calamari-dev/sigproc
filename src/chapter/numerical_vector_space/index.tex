...
一方で（特に信号解析では），\(\vect{x}\)の性質を調べるのに利用したい正規直交系\(\basis{B}\)があって，そこから\(V=\spannedby\basis{B}\)への直交射影\(\proj_V(\vect{x})\)を作ることも多い．そのような場合，直交射影は2つの操作に分解できる．
...
集合\(\basis{B}=\Set{\vect{e}_1,\dots,\vect{e}_m}\)を\(\numset{C}^n\)の（基底とは限らない）正規直交系とし，\(V=\spannedby\basis{B}\)とおく．このとき
\[
...
\]
なので，\(\proj_V(\vect{x})\)は\(m\times n\)行列\(\mat{W}=\htrps{\inlinevec{\vect{e}_1 & \cdots & \vect{e}_m}}\)により\(\proj_V(\vect{x})=\htrps{\mat{W}}\mat{W}\vect{x}\)と表せる．
言い換えれば，写像\(T\colon\numset{C}^n\to\numset{C}^m\)，\(\synth{T}\colon\numset{C}^m\to V\)を
\[
  T(\vect{x}) = \mat{W}\vect{x} = \matrice*{\innerp{\vect{x}}{\vect{e}_1} \\ \vdots \\ \innerp{\vect{x}}{\vect{e}_m}},
  \quad\synth{T}(\vect{y}) = \htrps{\mat{W}}\vect{y} = \sum_{i=1}^my_i\vect{e}_i
  \quad(\vect{y}=\trps{\matrice{y_1 & \cdots & y_m}})
\]
で定義すると\(\proj_V=\synth{T}T\)が成立する．
