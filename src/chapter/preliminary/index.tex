...
\begin{proposition}{}{monotone_converge}
  ...
  \begin{enumerate}
    \item 集合\(\Set{a_1,a_2,\dotsc}\)が上に有界である
    \item \(\seq{a_n}_{n\in\numset{N}}\)は単調増加する．すなわち，\(a_1\leq a_2\leq\dotsb\)である
  \end{enumerate}
\end{proposition}

\begin{proof}
  \(S=\Set{a_1,a_2,\dotsc}\)，\(\alpha=\sup S\)とする．このとき\(a_n\to\alpha\)（\(n\to\infty\)）であることを示す．
  任意に\(\varepsilon>0\)をとる．\cref{proposition:sup_inf_and_eps}より，\(x+\varepsilon>\alpha\)を満たすように\(x\in S\)をとれる．
  また，\(S\)の定義から\(a_N=x\)となる\(N\in\numset{N}\)が少なくとも1つある．
  \(n\geq N\)なら，\(a_N\leq a_n\leq\alpha\)より\(\abs{a_n-\alpha}=\alpha-a_n\leq\alpha-a_N<\varepsilon\)である．
...
\begin{theorem}{ボルツァーノ・ワイエルシュトラスの定理}{}\index{ぼるつぁーのわいえるしゅとらすのていり@ボルツァーノ・ワイエルシュトラスの定理}
  有界な実数列は収束する部分列を持つ．すなわち，数列\(\seq{x_n}_{n\in\numset{N}}\)の項全体が有界集合であるとき，狭義単調増加する自然数列\(\seq{\phi(n)}_{n\in\numset{N}}\)が存在して，数列\(\seq{x_{\phi(n)}}_{n\in\numset{N}}\)は収束列になる．
\end{theorem}
...
\begin{proof}
  ...
  \]
  で定義する．このとき\(I_1\supset I_2\supset\dotsb\)，\(b_n-a_n=2^{1-n}(b_1-a_1)\to 0\)（\(n\to\infty\)）なので，\cref{proposition:nested_intervals}より\(\bigcap_{n=1}^\infty I_n=\Set{\alpha}\)となる実数\(\alpha\)がある．
  ...
  \(f^{-1}(I_n)\)は無限集合だから，狭義単調増加する自然数列\(\seq{\phi(n)}_{n\in\numset{N}}\)を
  \[
    \phi(1) = 1,
    \quad\phi(n) = \min\Set{k\in f^{-1}(I_n)\given k>\phi(n-1)}\quad(n=2,3,\dotsc)
  \]
  で定義できる．\(\alpha\in I_n\)，\(x_{\phi(n)}=f(\phi(n))\in I_n\)より\(\abs{x_{\phi(n)}-\alpha}\leq b_n-a_n\to 0\)（\(n\to\infty\)），\(x_{\phi(n)}\to\alpha\)（\(n\to\infty\)）である．
\end{proof}
