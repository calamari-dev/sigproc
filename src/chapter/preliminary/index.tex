...
\begin{definition}{上界，下界}{}\index{じょうかい@上界}\index{かかい@下界}
  \(S\)を\(\numset{R}\)の部分集合とする．
  \begin{enumerate}
    \item 実数\(M\)が\(S\)の\termdef{上界}（upper bound）であるとは，任意の\(x\in S\)に対して\(x\leq M\)が成立することをいう
    \item 実数\(m\)が\(S\)の\termdef{下界}（lower bound）であるとは，任意の\(x\in S\)に対して\(x\geq m\)が成立することをいう
  \end{enumerate}
\end{definition}

\begin{definition}{上限，下限}{}\index{じょうげん@上限}\index{かげん@下限}\index{\(\sup S\)}\index{\(\inf S\)}
  \(S\)を\(\numset{R}\)の空でない部分集合とする．\(S\)の上界の全体集合を\(U\)，下界の全体集合を\(L\)とおく．
  \begin{enumerate}
    \item \(U\)が空でないとき（つまり\(S\)の上界が存在すれば），\(U\)は最小元\(\min U\)を持つ．\(\min U\)を\(S\)の\termdef{上限}（supremum）といい，\(\sup S\)と書く
    \item \(L\)が空でないとき（つまり\(S\)の下界が存在すれば），\(L\)は最大元\(\max L\)を持つ．\(\max L\)を\(S\)の\termdef{下限}（infimum）といい，\(\inf S\)と書く
  \end{enumerate}
\end{definition}
