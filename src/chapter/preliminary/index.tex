...
\begin{definition}{部分空間の和}{sum_of_subspace}\index{わぶぶんくうかん@和（部分空間）}\index{ぶぶんくうかん@部分空間!のわ@—の和}\index{\(W_1+W_2\)}
...
特に\(W_1\cap W_2=\Set{\zvec}\)であるとき，\(W_1+W_2\)を\(W_1\)と\(W_2\)の\termdef{直和}\index{ちょくわぶぶんくうかん@直和（部分空間）}\index{ぶぶんくうかん@部分空間!のちょくわ@—の直和}（direct sum）という．
...
一般に，ベクトル空間\(V\)の部分集合\(S\)に対して，\(S\)の元の線型結合で書けるベクトルの全体集合を\(S\)が\termdef{生成する部分空間}\index{せいせいぶぶんくうかん@生成（部分空間）}\index{ぶぶんくうかん@部分空間!せいせいする@生成する—}（generated subspace）といい，
...
\begin{figure}[htbp]
  ...
  \end{minipage}
  \caption{\(\vect{v}_1,\vect{v}_2,\vect{v}_3\in T\)の線型結合で\(\vect{x}=\trps{\matrice{3/2 & 0}}\)を表した様子．明らかに\(\vect{x}=(-3/2)\vect{v}_3\)である一方，\(\vect{x}=(\vect{v}_1+\vect{v}_2)/2=(1/2)\vect{v}_1+(1/2)\vect{v}_2\)も成り立つ．}
...
\begin{proposition}{}{basis_and_matrix}
  ...
  \begin{enumerate}
    ...
    \item 行列\(\inlinevec{\vect{v}_1 & \cdots & \vect{v}_n}\)は正則である
  \end{enumerate}
\end{proposition}
...
\begin{proposition}{}{gram_schmidt}
  ...
  \[
    \vect{u}_1 = \vect{v}_1,
    \quad\vect{u}_i = \vect{v}_i-\sum_{k=1}^{n-1}\frac{\innerp{\vect{v}_i}{\vect{u}_k}}{\innerp{\vect{u}_k}{\vect{u}_k}}\vect{u}_k\quad(i=2,\dots,n)
  \]
  でベクトル\(\vect{u}_1,\dots,\vect{u}_n\)を定義すると，集合\(\narrowbaselines\Set{\vect{u}_i/\sqrt{\innerp{\vect{u}_i}{\vect{u}_i}}\given i=1,\dots,n}\)は正規直交系になる．
\end{proposition}
...
\begin{definition}{線型写像}{linear_mapping}\index{せんけいしゃぞう@線型写像}
  ...
  \begin{enumerate}
    ...
    \item 任意の\(\vect{x}\in V\)，\(c\in\numset{K}\)に対して\(f(c\vect{x})=cf(\vect{x})\)である
  \end{enumerate}
\end{definition}
...
\begin{proof}
  ...
  任意に\(\vect{x}\in E_{\lambda_1}(\mat{A})\cap E_{\lambda_2}(\mat{A})\)をとる．\(\mat{A}\vect{x}=\lambda_1\vect{x}=\lambda_2\vect{x}\)より\((\lambda_1-\lambda_2)\vect{x}=\zvec\)であり，\(\lambda_1\neq\lambda_2\)なので\(\vect{x}=\zvec\)である．
  よって，\(E_{\lambda_1}(\mat{A})\cap E_{\lambda_2}(\mat{A})\)は\(\zvec\)以外に元を持たない．
\end{proof}
...
\begin{proof}
  ...
  \(\mat{P}=\inlinevec{\vect{v}_1 & \cdots & \vect{v}_n}\)とおく．このとき，各\(\vect{v}_i\)に対応する固有値を\(\lambda_i\)とおくと
  \(\mat{A}\mat{P}=\inlinevec{\mat{A}\vect{v}_1 & \cdots & \mat{A}\vect{v}_n}=\inlinevec{\lambda_1\vect{v}_1 & \cdots & \lambda_n\vect{v}_n}\)
  ...
  \(\mat{P}=\inlinevec{\vect{v}_1 & \cdots & \vect{v}_n}\)，\(\mat{\Lambda}=\diag(\lambda_1,\dots,\lambda_n)\)とおく．
  このとき\(\inlinevec{\mat{A}\vect{v}_1 & \cdots & \mat{A}\vect{v}_n}=\mat{A}\mat{P}=\mat{P}\mat{\Lambda}=\inlinevec{\lambda_1\vect{v}_1 & \cdots & \lambda_n\vect{v}_n}\)なので，
