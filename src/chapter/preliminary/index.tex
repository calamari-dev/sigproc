...
\begin{example}
  ...
  通常の加法とスカラー乗法によって，\(\numset{K}\)上のベクトル空間になる．ただし，\(\trps{\mat{A}}\)\index{\(\trps{\mat{A}}\)}は行列\(\mat{A}\)の転置行列を意味する．
\end{example}
...
一般に，ベクトル空間\(V\)の部分集合\(S\)に対して，\(S\)の元の線型結合で書けるベクトルの全体集合を\(S\)が\termdef{生成する部分空間}\index{せいせいするぶぶんくうかん@生成する部分空間}\index{ぶぶんくうかん@部分空間!せいせいする@生成する——}（generated subspace）といい，
...
以下，対角行列\(\begin{bsmallmatrix}a_1 & & \\ & \ddots & \\ & & a_n\end{bsmallmatrix}\)を\(\diag(a_1,\dots,a_n)\)\index{\(\diag(a_1,\dots,a_n)\)}と略記する．
...
ここではε‐δ論法による極限の定義を既知としたうえで，実数の性質からしたがう重要な事実をいくつか挙げる．
...
\begin{definition}{上界，下界}{}\index{じょうかい@上界}\index{かかい@下界}
  ...
  \begin{enumerate}
    ...
    \item 実数\(b\)が\(X\)の\termdef{下界}（lower bound）であるとは，任意の\(x\in X\)に対して\(x\geq b\)が成立することをいう
  \end{enumerate}
\end{definition}

\(X\)の上界が存在するとき，\(X\)は上に有界であるという．同様に，\(X\)の下界が存在するとき，\(X\)は下に有界であるという．\(X\)が上にも下にも有界であるときは，単に「有界である」という．

\begin{definition}{上限，下限}{}\index{じょうげん@上限}\index{かげん@下限}\index{\(\sup X\)}\index{\(\inf X\)}
...
上限と下限について議論するときは，次の\cref{proposition:sup_inf_and_eps}が便利である．
...
\begin{proof}
  ...
  \(x\in\bigcap_{n=1}^\infty I_n\)を任意にとる．\(x\)は\(A\)の上界なので，\(a_n\leq\alpha\leq x\leq b_n\)，\(0\leq x-\alpha\leq b_n-a_n\)である．
...
\begin{theorem}{ボルツァーノ・ワイエルシュトラスの定理}{bolzano_weierstrass}\index{ぼるつぁーのわいえるしゅとらすのていり@ボルツァーノ・ワイエルシュトラスの定理}
  有界な実数列は収束する部分列を持つ．すなわち，実数列\(\seq{x_n}_{n\in\numset{N}}\)の項全体が有界集合であるとき，\(\phi(1)<\phi(2)<\dotsb\)を満たす自然数列\(\seq{\phi(n)}_{n\in\numset{N}}\)が存在して，数列\(\seq{x_{\phi(n)}}_{n\in\numset{N}}\)は収束列になる．
\end{theorem}

\begin{proof}
  写像\(f\colon\numset{N}\to\numset{R}\)を\(f(n)=x_n\)で定義する．また，\(x_1,x_2,\dotsc\in I_1\)となる有界閉区間\(I_1=\ccival{a_1}{b_1}\)を1つ選び，閉区間\(I_n=\ccival{a_n}{b_n}\)（\(n=1,2,\dotsc\)）を帰納的に
  \[
  ...
  \]
  で定義する．このとき\(I_1\supset I_2\supset\dotsb\)，\(b_n-a_n=2^{1-n}(b_1-a_1)\to 0\)（\(n\to\infty\)）なので\footnote{「\(2^{-n}\to 0\)（\(n\to\infty\)）」も実数の性質からしたがう定理であるが，ここでは認める．}，
  ...
  以上により，\(f^{-1}(I_n)\)は無限集合である．よって，自然数列\(\seq{\phi(n)}_{n\in\numset{N}}\)を
  \[
    \phi(1) = 1,
    \quad\phi(n) = \min\Set{k\in f^{-1}(I_n)\given k>\phi(n-1)}\quad(n=2,3,\dotsc)
  \]
  で定義できる．\(\alpha\in I_n\)，\(x_{\phi(n)}=f(\phi(n))\in I_n\)だから\(\abs{x_{\phi(n)}-\alpha}\leq b_n-a_n\to 0\)（\(n\to\infty\)），\(x_{\phi(n)}\to\alpha\)（\(n\to\infty\)）である．
\end{proof}

\subsection{コーシー列}

\begin{definition}{コーシー列}{real_cauchy_sequence}\index{こーしーれつ@コーシー列}
  \(\seq{x_n}_{n\in\numset{N}}\)は実数列とする．\(\seq{x_n}_{n\in\numset{N}}\)が\termdef{コーシー列}（Cauchy sequence）であるとは，
...
\begin{proof}
  実数列\(\seq{x_n}_{n\in\numset{N}}\)はコーシー列であるとする．このとき，条件「\(m,n>N\implies\abs{x_m-x_n}<1\)」を満たす\(N\in\numset{N}\)がある．
  \(m>N\)なら\(\abs{x_m-x_N}<1\)だから，\(x_{N+1},x_{N+2},\dotsc\)はすべて開区間\(I=\ooival{x_N-1}{x_N+1}\)上にある．
  \(S=\Set{x_1,\dots,x_N}\)とする．このとき集合\(\Set{x_1,x_2,\dotsc}\)は\(\ccival{\min S}{\max S}\cup I\)の部分集合なので，有界である．
  したがって，\cref{theorem:bolzano_weierstrass}より収束する部分列\(\seq{x_{\phi(n)}}_{n\in\numset{N}}\)がある．この極限値を\(\alpha\)とおくと，
  \(\abs{x_n-\alpha}\leq\abs{x_n-x_{\phi(n)}}+\abs{x_{\phi(n)}-\alpha}\to 0\)（\(n\to\infty\)）より\(x_n\to\alpha\)（\(n\to\infty\)）である．
\end{proof}
