...
\(\numset{R}^2\)において，ベクトルのなす角はドット積\((x_1,x_2)\cdot(y_1,y_2)=x_1y_1+x_2y_2\)から計算できた．
次の定義は，ドット積が持つ性質を捨象したものである．
...
ここでは\(\varepsilon\)‐\(N\)論法による極限の定義を既知として，実数の性質からしたがう事実をいくつか挙げる．
...
\(X\)が上にも下にも有界であるときは，単に「\termdef{有界}である」という．
...
数列の各項からなる集合が有界であるとき，数列は有界であるという．
...
\cref{proposition:monotone_converge}より，\(\limsup_{n\to\infty}a_n<+\infty\)と\(\sup\Set{a_1,a_2,\dotsc}<+\infty\)は同値であり，
同様のことが\(\liminf_{n\to\infty}a_n\)についても成り立つ．上極限と下極限について，次のことが知られている．
...
実数列について，数列がコーシー列であることと収束列であることは同値である．ただし，コーシー列であることは極限値を使わないで確かめられる．

\begin{example}
  \(\sqrt{2}=1{.}41421356\dotso\)の小数点以下第\(n\)位まで拾った有理数を\(a_n\)とする．
  すなわち\(a_1=1{.}4\)，\(a_2=1{.}41\)のようにする．このとき\(\abs{a_m-a_n}\leq 10^{-\min\Set{m,n}}\to 0\)（\(m,n\to\infty\)）だから，\(\seq{a_n}_{n\in\numset{N}}\)はコーシー列である．
\end{example}
