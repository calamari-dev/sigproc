...
\begin{proposition}{グラム・シュミットの直交化法}{gram_schmidt}\index{ぐらむしゅみっとのちょっこうかほう@グラム・シュミットの直交化法}
  ...
  \[
    \vect{u}_1 = \vect{a}_1,
    \quad\vect{u}_k = \vect{a}_k-\sum_{i=1}^{n-1}\frac{\innerp{\vect{a}_k}{\vect{u}_i}}{\innerp{\vect{u}_i}{\vect{u}_i}}\vect{u}_i\quad(k\geq 2)
  \]
...
\begin{proof}
  \(f\)が単射なら，\(f(\zvec)=f(\zvec+\zvec)=f(\zvec)+f(\zvec)\)より\(f(\zvec)=\zvec\)，\(\nul f=\Set{\zvec}\)である．逆に\(\nul f=\Set{\zvec}\)なら，\(\vect{v}_1,\vect{v}_2\in V\)が\(f(\vect{v}_1)=f(\vect{v}_2)\)を満たすとき
