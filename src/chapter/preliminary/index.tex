...
\begin{definition}{部分空間の和}{sum_of_subspace}\index{わぶぶんくうかんの@和，部分空間の}\index{ぶぶんくうかん@部分空間!わ@和}\index{\(V_1+V_2\)}
...
直和であることを強調したいときは，和\(W_1+W_2\)を\(W_1\oplus W_2\)\index{\(V_1\oplus V_2\)}とも書く．
...
\begin{definition}{上限，下限}{}\index{じょうげん@上限}\index{かげん@下限}\index{すーぷ@\(\sup S\)}\index{いんふ@\(\inf S\)}
...
\begin{proof}
  ...
  \]
  で定義する\footnote{写像\(f\colon X\to Y\)と集合\(A\subset X\)，\(B\subset Y\)に関して\(\ran{f}{A}=\Set{f(x)\given x\in A}\)\index{\(\ran{f}{S}\)}，\(\inv{f}{B}=\Set{x\in X\given f(x)\in B}\)\index{\(\inv{f}{S}\)}である．}．
