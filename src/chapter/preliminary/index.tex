...
この写像\(\phi\)は，次に定義する「線型写像」の一例である．

\begin{definition}{線型写像，線形変換}{linear_mapping}\index{せんけいしゃぞう@線型写像}\index{せんけいへんかん@線型変換}
  \(V\)と\(W\)を\(\numset{K}\)上のベクトル空間とする．以下の条件を満たす写像\(f\colon V\to W\)を\termdef{線型写像}（linear mapping）という．
