...
\begin{proof}
  ...
  \(m>N\)なら\(\abs{x_m-x_{N+1}}<1\)だから，集合\(\Set{x_{N+1},x_{N+2},\dotsc}\)は有界である．
  よって，\cref{theorem:bolzano_weierstrass}より数列\(\seq{x_{N+n}}_{n\in\numset{N}}\)は収束する部分列\(\seq{x_{N+\phi(n)}}_{n\in\numset{N}}\)を持つ．
  この極限値を\(\alpha\)とおくと，\(\abs{x_n-\alpha}\leq\abs{x_n-x_{N+\phi(n)}}+\abs{x_{N+\phi(n)}-\alpha}\to 0\)（\(n\to\infty\)）より\(x_n\to\alpha\)（\(n\to\infty\)）である．
\end{proof}
