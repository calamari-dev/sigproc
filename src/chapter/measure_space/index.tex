...
\begin{note}
  ...
  また，集合\(A\subset\Omega\)が\cref{equation:caratheodory_measurable}の\(\sigmaalg{F}\)に属するとき\texttwoemdash つまり，任意の\(E\in\powerset{\Omega}\)に対し\(\mu^{\mathord{\ast}}(A)=\mu^{\mathord{\ast}}(A\cap E)+\mu^{\mathord{\ast}}(A\setminus E)\)であるとき\texttwoemdash
...
\begin{definition}{}{}\index{るいせきぶんぷかんすう@累積分布関数}\index{かくりつみつどかんすう@確率密度関数}\index{かくりつしつりょうかんすう@確率質量関数}\index{CDF|see{累積分布関数}}\index{PDF|see{確率密度関数}}\index{PMF|see{確率質量関数}}
  ...
  \begin{enumerate}
    \item 関数\(F_X(x)=\prob(\inv{X}{\ocival{-\infty}{x}})\)を\(X\)の\termdef{累積分布関数}（cumulative distribution function; CDF）という．
    \item 像\(\ran{X}{\Omega}\)が有限または可算集合のとき，関数\(p_X(x)=\prob(\inv{X}{x})\)を\(X\)の\termdef{確率質量関数}（probability mass function; PMF）という．
...
\[
  \prob(\inv{X}{A}) = \sum_{x\in A}p_X(x)
\]
が成立する．同様に，\(X\)が確率密度関数\(f_X\)を持てば
\[
  \prob(\inv{X}{A}) = \int_Af_X(x)\measured[x]{\lambda}\quad\text{(\(\lambda\)は\(\numset{R}\)上のルベーグ測度)}
\]
が成立する．
...
\begin{definition}{ほとんどいたるところ}{}\index{ほとんどいたるところ}\index{ae@a{.}e{.}}
  \(P(x)\)を\(\Omega\)の元\(x\)に関する条件とする．\(\mu(N)=0\)，\(\scomp{\Set{x\given P(x)}}\subset N\)を満たす集合\(N\in\sigmaalg{F}\)が存在するとき，
  \(P(x)\)は\termdef{ほとんどいたるところ}（almost everywhere; a{.}e{.}）成立するという．このことを\(\almosteverywhere[x\in\Omega]{P(x)}\)と書く．
\end{definition}
