...
\begin{definition}{可測関数}{}\index{かそくかんすう@可測関数}\index{fkasoku@\(\sigmaalg{F}\)‐可測}
...
確率変数\(\condexpval{X\given\sigmaalg{G}}\)をσ‐加法族\(\sigmaalg{G}\)の下での\(X\)の\termdef{条件つき期待値}（conditional expectation）という．
...
また，\(\Omega=\bigsqcup_{n\in\numset{Z}}\delta Y_n\)であるから，\(\prob(\delta Y_n)>0\)なら\(\condexpval{X\given\sigmaalg{G}_{\increment y}}\)は
\[
...
\]
で定義できる．この確率変数は集合\(\Set{Y=y}\)上で一定の値をとり，その値は条件\(\delta Y_n\)（つまり\(Y\fallingdotseq y\)）の下での\(X\)の期待値を表す．
