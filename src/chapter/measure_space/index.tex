...
\begin{definition}{可測関数}{}\index{かそくかんすう@可測関数}
  \((\Omega,\sigmaalg{F})\)を可測空間とする．写像\(f\colon\Omega\to\extendedreal\)が\termdef{可測関数}（measurable function）であるとは，
...
\[
  \int f\measured{\mu} = \sup\Set*{\int\phi\measured{\mu}\given\text{\(\phi\)は\(0\leq\phi\leq f\)を満たす単関数}}
\]
で定義する．ただし\(\phi\leq f\)とは，任意の\(x\in\Omega\)に対して\(\phi(x)\leq f(x)\)が成り立つことを意味する．

\begin{note}
  より具体的に，\(\int f\measured{\mu}\)を非負値単関数の積分の極限で表すこともできる．
...
\(f\)が負の値をとりうる可測関数のときは，\(f^{\mathord{+}}(x)=f(x)\indicator_{\inv{f}{\ccival{0}{+\infty}}}(x)\)と\(f^{\mathord{-}}(x)=-f(x)\indicator_{\inv{f}{\ccival{-\infty}{0}}}(x)\)が非負値可測関数であることを利用して
\[
...
\[
  \int_Sf\measured{\mu} = \int f(x)\indicator_S(x)\measured[x]{\mu}
\]
で定義される．

特に\((\Omega,\sigmaalg{F},\prob)\)が確率空間のとき，確率変数\(X\)の積分\(\int_\Omega X(\omega)\measured[\omega]{\prob}\)を\(X\)の\termdef{期待値}\index{きたいち@期待値}\index{EX@\(\expval[X]\)}（expected value）といい，\(\expval[X]\)と書く．
...
\begin{proposition}{}{}
  実数値関数\(f\)は有界閉区間\(I=\ccival{a}{b}\)上で定義され有界とする．このとき，\(f\)がリーマン積分できれば\(f\)は可積分であり
  \[
