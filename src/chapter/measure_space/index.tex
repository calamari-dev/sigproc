...
\begin{definition}{生成するσ‐加法族}{}\index{せいせい@生成!しぐまかほうぞく@σ‐加法族}\index{しぐまかほうぞく@σ‐加法族!せいせいする@生成する\texttwoemdash}\indexsymbol{\(\sigmagen{\symcal{S}}\)}
  ...
  \]
  は\(\Sigma(\symcal{S})\)に属し，\(\Sigma(\symcal{S})\)の任意の元は\(\sigmagen{\symcal{S}}\)を包含する．\(\sigmagen{\symcal{S}}\)を\(\symcal{S}\)が\termdef{生成するσ‐加法族}（generated σ‐algebra）という．
\end{definition}
...
このことを\(Y=Y'\almostsurely\)\index{ほとんどかくじつに@ほとんど確実に}\index{as@\(\almostsurely\)}と表す\footnote{\(\almostsurely\)はalmost surely（ほとんど確実に）の略．}．
...
\begin{definition}{ほとんどいたるところ}{}\index{ほとんどいたるところ}\index{ae@\(\almosteverywhere\)}
  \(C(\omega)\)を変数\(\omega\)に関する条件とする．\(\mu(N)=0\)，\(\scomp{\Set{\omega\in\Omega\given C(\omega)}}\subset N\)を満たす集合\(N\in\sigmaalg{F}\)が存在するとき，
  \(C(\omega)\)は\termdef{ほとんどいたるところ}（almost everywhere; a{.}e{.}）成立するという．このことを\(C(\omega)\almosteverywhere[\mu]\omega\in\Omega\)と書く．
  誤解のおそれがなければ，\(C(\omega)\almosteverywhere\omega\)や\(C\almosteverywhere\)などと略記する．
\end{definition}
...
\begin{equation}
  ...
  \int\abs{f(x)+g(x)}^p\measured[x]{\mu} \leq \int(\abs{f(x)}^p+\abs{g(x)}^p)\measured[x]{\mu}
\end{equation}
は\termdef{ミンコフスキーの不等式}\index{みんこふすきーのふとうしき@ミンコフスキーの不等式}（Minkowski inequality）と呼ばれる．

しかし厄介なことに，\(\vnorm{\holder}\)が任意の\(f\in\symcal{L}^p\)に対して\(\vnorm{f}=0\iff f=0\)を満たすのは，\(\mu\)が特殊な測度のときに限られる．一方で，次の事実がある．
...
\cref{proposition:kernel_of_integration}から，\(\vnorm{f}=0\)となるのは\(\abs{f(x)}^p=0\almosteverywhere x\)，すなわち\(f=0\almosteverywhere\)のときだけである．
そのため，ほとんどいたるところで等しい値をとる関数を同一視すれば，\(\symcal{L}^p\)はノルム空間になる．

この同一視を数学の言葉で書くと，以下のようになる．集合\(N\)を\(N=\Set{\varphi\given\vnorm{\varphi}=0}=\Set{\varphi\given\varphi=0\almosteverywhere}\)で定義する．
また，各\(f\in\symcal{L}^p\)に対して集合\(\Set{f+\varphi\given\varphi\in N}\)を\(f+N\)とおく．このとき，任意の可測関数\(f,g\)について
\[
  g(x) = f(x)\almosteverywhere x
  \iff g-f \in N
  \iff g = f+(g-f)
  \in f+N
\]
だから，\(f+N\)はほとんどいたるところで値が\(f\)と等しい可測関数の全体集合である．
しかも，\(\varphi\in N\)なら\cref{equation:minkowski}より\(\vnorm{f}=\vnorm{(f+\varphi)-\varphi}\leq\vnorm{f+\varphi}\leq\vnorm{f}\)，\(\vnorm{f+\varphi}=\vnorm{f}\)である．
よって，集合\(\Lebesgue{p}=\Set{f+N\given f\in\symcal{L}^p}\)の元\(F\)に対して，\(\vnorm{F}_p\)を
\[
  \vnorm{F}_p = \vnorm{f}
  = \int\abs{f(x)}^p\measured[x]{\mu}\quad\text{(\(f\)は\(F\)の任意の元)}
\]
で定義できる．\(\Lebesgue{p}\)は加法\((f+N)+(g+N)=(f+g)+N\)，スカラー乗法\(a(f+N)=(af)+N\)に関するベクトル空間であり，\(\vnorm{\holder}_p\)は\(\Lebesgue{p}\)のノルムである．

実は，\(\Lebesgue{p}\)はノルム\(\vnorm{\holder}_p\)によってバナッハ空間になる．バナッハ空間\(\Lebesgue{p}\)を\termdef{\(\Lebesgue{p}\)空間}という．

\begin{definition}{\(\Lebesgue{p}\)空間}{lp_space}\index{Lpkukan@\(\Lebesgue{p}\)空間}
  任意の可測関数\(f\)について，\([f]\)を\([f]=\Set{g\given g(x)=f(x)\almosteverywhere x}\)で定義する．
  ...
  また，ノルム\(\vnorm{\holder}_p\)を
  \[
    \vnorm{[f]}_p = \pqty*{\int\abs{f(x)}^p\measured[x]{\mu}}^{1/p}
  \]
...
\begin{note}
  細かいことを言うと，この定義はいわゆる「well‐definedness」が問題になるが，本書では深く考えないことにする．
\end{note}
...
\begin{example}
  ...
  実際，関数\(f\colon\numset{N}\to\numset{K}\)が\(\mu\)‐可測なら，関数\(\phi^{\mathord{+}}_n=\sum_{k=1}^nf^{\mathord{+}}(k)\indicator_{\Set{k}}\)は非負値単関数であり，\nameref{theorem:monotone_convergence}から
  \[
  ...
  \[
    ...
    = \sum_{n=1}^\infty f(n)
  \]
  である．よって\(\int\abs{f(n)}^p\measured[n]{\mu}=\sum_{n=1}^\infty\abs{f(n)}^p\)なので，\(\lebesgue{p}(\numset{N})=\Lebesgue{p}(\numset{N},\powerset{\numset{N}},\mu)\)である．
\end{example}
