...
しかし，たとえば\(\sigmaalg{B}(\numset{R})\)上の測度を定める場合，この方法は使えない．そうした状況では代わりに，σ‐加法族よりも考えやすい集合族の上で測度の雛形を作り，それをσ‐加法族全体へと拡張する．
...
\(\int f^{\mathord{-}}\measured{\mu}\)と\(\int f^{\mathord{+}}\measured{\mu}\)がどちらも有限なとき，\(f\)は\termdef{可積分}\index{かせきぶん@可積分}（integrable）であるという．また，集合\(S\in\sigmaalg{F}\)上での積分は
\[
...
\begin{note}
  ...
  \[
    \int_{\ooival{0}{+\infty}}\max\Set{f(x),0}\measured[x]{\lambda} = \int_{\ooival{0}{+\infty}}\max\Set{-f(x),0}\measured[x]{\lambda}
    = +\infty
  \]
  であり，ルベーグ積分できない．
\end{note}
...
確率論ではしばしば，\(\extendedreal\)の元\(x\)に関する条件\(C(x)\)と確率変数\(X\)に対して，集合\(\Set{\omega\given C(X(\omega))}\)を\(\Set{C(X)}\)と書く．
たとえば\(\Set{X=1}=\Set{\omega\given X(\omega)=1}=\inv{X}{1}\)，\(\prob(X=Y)=\prob(\Set{\omega\given X(\omega)=Y(\omega)})\)である．
...
写像\(\prob^X\colon\borelset{\numset{R}}\to\ccival{0}{+\infty}\)を\(\prob^X(A)=\prob(X\in A)\)で定義すると，\(\prob^X\)は\(\borelset{\numset{R}}\)上の確率測度になる．
\(\prob^X\)を\(X\)の\termdef{確率分布}\index{かくりつぶんぷ@確率分布}\index{PX@\(\prob^X\)}（probability distribution），もしくは\(X\)による\(\prob\)の\termdef{像測度}\index{ぞうそくど@像測度}\index{そくど@測度!ぞう@像\texttwoemdash}（image measure）という．
\(X\)が確率質量関数\(p_X\)を持つとき，任意の\(A\in\borelset{\numset{R}}\)に対して
\[
  \prob^X(A) = \sum_{x\in A}p_X(x)
\]
が成立する．同様に，\(X\)が確率密度関数\(f_X\)を持つとき
\[
  \prob^X(A) = \int_Af_X(x)\measured[x]{\lambda}\quad\text{(\(\lambda\)は\(\numset{R}\)上のルベーグ測度)}
\]
が成立する．

これらの関数は，期待値を計算するときに重用される．\(\numset{R}\)上で定義された可測関数\(f\)に関して\(\expval[f(X)]=\int f(X(\omega))\measured[\omega]{\prob}\)であるが，
この値を計算するときは\(X(\omega)\to x\)，\(1\measured[\omega]{\prob}\to f_X(x)\measured[x]{\lambda}\)と置き換えて，\(\int f(x)f_X(x)\measured[x]{\lambda}\)を計算すればよい．
...
このことを\(\almostsurely{Y=Y'}\)\index{ほとんどかくじつに@ほとんど確実に}\index{as@a{.}s{.}}と表す\footnote{\(\almostsurely{}\)はalmost surely（ほとんど確実に）の略．}．

また，\(\indicator_A\)（\(A\in\sigmaalg{F}\)）の条件つき期待値\(\condexpval{\indicator_A\given\sigmaalg{G}}\)を\(\sigmaalg{G}\)の下での\(A\)の\termdef{条件つき確率}\index{じょうけんつきかくりつ@条件つき確率}（conditional probability）といい，\(\condprob{A\given\sigmaalg{G}}\)\index{PAG@\(\condprob{A\given\sigmaalg{G}}\)}と書く．

\section{\texorpdfstring{\(\Lebesgue{p}\)空間}{Lp空間}}

\((\Omega,\sigmaalg{F},\mu)\)を測度空間とする．

\begin{definition}{ほとんどいたるところ}{}\index{ほとんどいたるところ}\index{ae@a{.}e{.}}
  \(C(x)\)を\(\Omega\)の元\(x\)に関する条件とする．\(\mu(N)=0\)，\(\scomp{\Set{x\given C(x)}}\subset N\)を満たす集合\(N\in\sigmaalg{F}\)が存在するとき，
  \(C(x)\)は\termdef{ほとんどいたるところ}（almost everywhere; a{.}e{.}）成立するという．このことを\(\almosteverywhere[x\in\Omega]{C(x)}\)と書く．
\end{definition}

可測関数間の同値関係\(\sim\)を\(f\sim g\iff\almosteverywhere[x\in\Omega]{f(x)=g(x)}\)で定義する．

\begin{definition}{\(\Lebesgue{p}\)空間}{lp_space}\index{Lpkukan@\(\Lebesgue{p}\)空間}
  \((\Omega,\sigmaalg{F},\mu)\)を測度空間とする．各\(p\in\coival{1}{+\infty}\)に対し，\(\sim\)に関する可測関数\(f\)の同値類\([f]\)で
  \[
    \vnorm{[f]}_p = \pqty*{\int_\Omega\abs{f(x)}^p\measured[x]{\mu}}^{1/p}
  \]
  の値が有限であるものの全体集合を\(\Lebesgue{p}(\Omega,\sigmaalg{F},\mu)\)とおく．このとき，\(\Lebesgue{p}(\Omega,\sigmaalg{F},\mu)\)は\(\vnorm{\holder}_p\)をノルムとしてバナッハ空間になる．
...
\begin{proposition}{}{}
  ...
  \[
    \almostsurely{\condexpval{X\given\sigmaalg{G}} = \proj_V(X)}\quad(V=\Lebesgue{2}(\Omega,\sigmaalg{G},\prob))
  \]
\end{proposition}
