...
この空間を\(\symcal{L}^p=\symcal{L}^p(\Omega,\sigmaalg{F},\mu)\)としよう．\termdef{ミンコフスキーの不等式}\index{みんこふすきーのふとうしき@ミンコフスキーの不等式}（Minkowski inequality）
\begin{equation}
  \label{equation:minkowski}
  \int\abs{f(x)+g(x)}^p\measured[x]{\mu} \leq \int(\abs{f(x)}^p+\abs{g(x)}^p)\measured[x]{\mu}
\end{equation}
から，\(\symcal{L}^p\)はベクトル空間である．また，\(\vnorm{\holder}\)を
\[
  \vnorm{f} = \pqty*{\int\abs{f(x)}^p\measured[x]{\mu}}^{1/p}\quad(f\in\symcal{L}^p)
\]
で定義すると，\(\vnorm{\holder}\)はノルムの条件のうち\(\vnorm{af}=\abs{a}\vnorm{f}\)，\(\vnorm{f+g}\leq\vnorm{f}+\vnorm{g}\)を満たす．
...
よって，集合\(\Lebesgue{p}=\Set{f+N\given f\in\symcal{L}^p}\)の元\(F\)に対して，\(\vnorm{F}_p\)を\(\vnorm{F}_p=\vnorm{f}=(\int\abs{f(x)}^p\measured[x]{\mu})^{1/p}\)で定義できる．ただし，\(f\)は\(F\)の任意の元である．
\(\Lebesgue{p}\)は加法\((f+N)+(g+N)=(f+g)+N\)，スカラー乗法\(a(f+N)=(af)+N\)に関するベクトル空間で，\(\vnorm{\holder}_p\)は\(\Lebesgue{p}\)のノルムである．
...
\begin{example}
  ...
  \[
    ...
    = \sum_{n=1}^\infty f(n)
  \]
  である．よって\((\int\abs{f(n)}^p\measured[n]{\mu})^{1/p}=(\sum\abs{f(n)}^p)^{1/p}\)，\(\lebesgue{p}(\numset{N})=\Lebesgue{p}(\numset{N},\powerset{\numset{N}},\mu)\)である．
\end{example}
